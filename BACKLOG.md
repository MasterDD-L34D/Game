...
- **FASE 1 -- chiudi il design** (verify-first 2026-06-20: H4/H8/H9 risultati GIA chiusi, marker line stale anti-pattern #19): **aperti reali = solo master-dd-gated** -> **H2** economy combat cost-gate (`cost_sg/pp/pt` decorativi vs SoT pools, verdetto master-dd, spec #2530-A2) + **DECISIONE H1** GAP-C fase-3/4 (build-vs-gate) + **H7** pillar re-ratifica (`PILLAR-LIVE-STATUS.md` 19gg stale: giugno non riflesso -- item-1 17/17 + SPEC-J/K/H + dossier; nessun surface-gap autonomo wireable: P1 azzerato / P5 design-call+Godot / P6 balance-value). **CHIUSI verify-first 2026-06-20**: H4 Cat F 7/7 wired + 38/38 test pass (#2538, `progressionApply.js`+3 test file) -- H8 1-HP comment GIA "RATIFIED master-dd 2026-06-02" (`symbiontBond.js:259`) -- H9 OD-022/OD-023 entrambi `status=resolved` (`OPEN_DECISIONS.md`).
...
- [ ] **Build zone-defense (lever D4 comportamentale)** -- spec RATIFICATA 2026-07-06 (`docs/planning/2026-07-06-sistema-zone-defense-spec-draft.md` sez. 6, PR #3238): intent NUOVO `defend_zone` decision-layer-only + unlock per-encounter authored (campo YAML opt-in, forbidden-path additivo da segnalare) + misura fattoriale 2x2 (x roster-scaling #3231) su variante capture_point dorsale, N=10 -> N=40 arm vincente + liveness gate. Prereq content: autorare la variante capture. Arco dedicato.
- [ ] **Flip `XP_BUDGET_GEOMETRY_ENABLED`** -- calibrazione D9 MERGED #3239 (actionEconomyScale + hazard gated su MOVE_TERRAIN_COST; 3 grid_sized da critical_over a in_band 1.11 = match misurato); fork **(a) lettura relativa RATIFIED** 2026-07-06. Flip = **staged-latent keys.env** (owner, host CODEMASTERDD, inerte fino a restart; cambia solo la riga warn a /start).
- [ ] **TKT-SIM-COOP-SEED**: `batch-ai-runner` -> ai-driven-sim NON pinna mai il combat RNG (il seed numerico va a coop `run_seed` = solo world-enricher, `coopOrchestrator.confirmWorld`; stesso pattern del bug #2561 fixato solo nel combat-adapter). Pairing by-seed dei batch coop = label-only. Fix = seam backend (finding PR #3240). Re-run paired delle bande pre-#3232 = DEFERRED (verdetto 2026-07-06: si fa gratis alla prossima modifica banda, L-069).
...
- [x] **W5 sim-harness AI-player** -- DELIVERED (inc-1/2/3-ER6 + D8 + W6 graded lane). 🔑 il graded metric MORDE solo su power-differential (D8/ER6 = NULL inerti; W6 = REAL). Handoff `docs/planning/2026-07-01-session-handoff-w5-d8-w6-close.md`. Residuo lane: D6 graded re-ratify · N3 ER7 flag-ON N=40 · G6 Godot (sotto).
- [x] **Form-pulse W6 graded ratify + staged-latent** DONE #3139 `92e52636` + #3143 `a0a9cce1`. Producer **party-normalized** (sqrt(nPlayers), PROD flag-gated) -> **un solo `w~0.78`** cross-party; **offset OVER-compensa offesa** (net enemy_hp +0.12 @1.4, survival-tilt irriducibile). **STAGED-LATENT keys.env** (flag + anchor 1.20 + w0.78, inerti). 🔴 **OWNER TODO**: (a) anchor **1.20** (staged, tua scelta) vs **1.15** (offense-null corretto post-seed-fix; classifier ha bloccato la sostituzione autonoma) -> cambia keys.env se vuoi 1.15; (b) flip = deploy prod `>=92e52636` + restart. Evidence `docs/playtest/2026-07-01-w6-form-pulse-v2-graded-cross-biome.md`. Residuo: per-pick imprint power (se serve balance) · tabella tupla->asse hash-mod (master-dd).
- [x] **D8 chain-lightning graded re-ratify** DONE #3136 `31c07902` = **NULL by non-exercise** (0 terrain reaction sparano in-sim; fire-count 0/N=40 + self-test + false-null Workflow). Flag `TERRAIN_CHAIN_LIGHTNING_ENABLED` resta OFF (band-neutral by non-exercise). 🔑 owner note: `magnetico_overload` (elettrico) esiste in content ma manca il water-tile producer -> D8 diventa vivo solo aggiungendo un canale acqua/water attack. Evidence `docs/playtest/2026-07-01-d8-chain-lightning-graded-reratify.md`.
- [ ] **D6 axis->trait grant graded re-ratify** (chip, gated su W5 -> ora sbloccato) -- imprint grant `dilatazione_temporale_percettiva` extra_damage; stesso approccio paired-graded (seed NUMERICO! [[lesson_numeric_seeds_paired_sim]]).
- [ ] **N3 SPEC-I ER7 flag-ON N=40** (chip) -- `BIOME_POPULATION_ENABLED`, build shipped #2723; graded.
...
- [x] **Borderline trait mechanics review** (#3118) -- **KEEP ratificato (verdetto owner 2026-07-02)**: coralli_sinaptici_fotofase / nodi_sinaptici_superficiali / scintilla_sinaptica restano as-built (zero esposizione encounter, descrizioni gia' patchate, status PROPOSED = swap-freely). Re-valutare quando frattura_abissale_sinaptica avra' encounter reali.
...
### 🔄 IN PROGRESS 2026-06-22 -- aa01 Sprint-Impronta RECONCILIATION (master-dd SUPERSEDE override)

**Override del verdetto SUPERSEDE 2026-06-07 sopra** (record preservato, additive): master-dd ha deciso di NON superseddere ciecamente -- il lavoro va CORRETTO e riconciliato nel canon corrente, nulla cancellato, valutato uno-a-uno. Branch `origin/aa01/cap-*` (13) PRESERVATI finche' integrati o superseded-con-record. Plan + harsh-review (10 findings): [`docs/planning/2026-06-22-aa01-impronta-reconciliation-plan.md`](docs/planning/2026-06-22-aa01-impronta-reconciliation-plan.md). Memory: `project_aa01_impronta_reconciliation.md`.

- **Track A (quick-win canon-neutral, 2 PR)**: A1 CAP-06 elevation inline -> `elevationDamageMultiplier` helper + regression test (branch `refactor/cap06-elevation-helper`); A2 CAP-04 `CHANGELOG.md` adopt + bring-current; A3 CAP-03 README L95 de-drift -> `docs/reports/PILLAR-LIVE-STATUS.md` SoT (branch `docs/aa01-impronta-reconciliation`). **CAP-02** flint-status.json = DROP (gitignored auto-snapshot #2927, branch preservato).
- **Track B (DONE 2026-06-22)**: B1 CAP-07 diff vs main -> **verdetto SUPERSEDED**. Ground-truth refuta il premise "gated su `ability.channel` assente": su main il sistema terrain e' gia' LIVE+STATEFUL (`session.tile_state_map` populate inline `session.js:1164-1195` + burst + TTL-decay `sessionRoundBridge.js:1155-1180`). Il bridge cap-07 = duplicato inferiore (camelCase `tileStateMap` collide, populator `applyTerrainReaction` MAI chiamato = dead). NO merge; branch preservato. Carve-out DEFERRED = chain-lightning propagation (`chainElectrified` multi-tile, design/balance call, NON merge meccanico).
- **Track C (RATIFICATA 2026-06-22 master-dd)**: C1 L'Impronta design spec **`active`** (`docs/planning/2026-06-22-aa01-cap11-limpronta-reconciliation-spec.md`). **3 design-call DECISE**: (1) biomeResolver -> **AFFINITY** (non-binding weight read-only, biome resta route-canon); (2) imprint -> **ADDITIVE separate-phase** beat (device-authority, no host-token, fasi separate, target Godot); (3) CAP-12 telemetry -> **DEFER** (inerte/orfano: no producer, route no-auth, no migration, Gate-5 dead; sia standalone sia fold inciampano nello stesso forbidden-path migration gate + servono producer+write-hook inesistenti). 🔴 **Correzione ground-truth**: il claim "allineato ADR-2026-06-07 pt3" (riga 185 sopra) e' SOVRASTIMATO -- l'unico ADR-2026-06-07 e' `device-authority-tv-mirror-canon` (no "pt3"; non sanziona la persistenza vcSnapshot/selectedForm; `selectedForm` non e' nemmeno id canonico backend). **C2 sequencing** (post-ratifica, ogni pezzo flag-gated, sottrae Track A; cap-11=umbrella, CAP-06 gia' #2958): C2-affinity (`biomeAffinity()` weights read-only, BLOCKED su consumption sub-call) -> C2-imprint (additive phase + `/campaign/start/v2` device-auth, BLOCKED su placement/prompt-UX/Godot sub-calls, prob. spec propria + chip GGv2) -> telemetry DEFERRED -> UX museum-ref. **affinity consumption DECISA** (master-dd 2026-06-22): **1+2 = hint cosmetico + peso route-vote**. 🔴 **Verify-first: C2-affinity NON buildabile standalone** -> (1) input = 4-axis imprint choices, inesistenti su main (le raccoglie l'imprint beat = C2-imprint) -> coupled; (2) nome `biomeAffinity` GIA' su main (`services/species/biomeAffinity.js` species<->biome combat) -> serve nome distinto (es. `imprintBiomeAffinity`); (3) route-vote consumer doppiamente-gated (`META_NETWORK_ROUTING` OFF + Godot-route-UI). Quindi affinity FOLDS into C2-imprint MVP (input+hint), route-vote = layer successivo post meta-flip. **Gating residuo = imprint sub-calls** (placement / prompt-UX / Godot) -> serve build-spec C2-imprint dedicato. MUST-NOT-REGRESS: biome route-canon, fasi `character_creation`+`world_setup` separate, device-authority, target Godot.
- **Track C2 BUILT + Deferred (2026-06-22)**: C2-imprint backend MVP **#2970 MERGED** (additive NON-gating device-authority beat + `imprintBiomeWeights` producer + cosmetic hint; flag `IMPRINT_BEAT_ENABLED` OFF, band-neutral; `coop-phase-validator` 2 P1+3 P2+1 P3 risolti; ~45 test + coop 235/235 + AI 554/554). CAP-06 **#2958 MERGED**. **Deferred tracker (SoT)** = [`docs/planning/2026-06-22-aa01-deferred-tracker.md`](docs/planning/2026-06-22-aa01-deferred-tracker.md) (mirror hub codemasterdd `docs/handoffs/`): **D1** Godot phone surface + hint chip **DONE** (GGv2 #531 merged; TV cinematic = follow-up) -> **D2** flip `IMPRINT_BEAT_ENABLED` = **NEXT gate** (consumer esiste -> solo playtest + master-dd, NON autonomo) · **D3** publicSessionView in-match field · **D4** auto-timer defaulting (master-dd) · **D5** route-vote affinity weighting (gated `META_NETWORK_ROUTING` flip) · **D6** axis->trait grant (non-band-neutral, spec separata) · **D7** prose/hint-string (master-dd HITL) · **D8** CAP-07 chain-lightning propagation (balance) · **D9** CAP-12 telemetry canon-home (spec futura; cross-ref SPEC-M Form-Pulse + reconstruct-from-ledger doctrine). aa01 = track proprio, NON SPEC-A..Q (cross-link solo D5=meta-network, D9=SPEC-M).
...
- **TUTTE le dir CURRENT-bumpabili battute (B1-B10).** Decisioni master-dd (AskUserQuestion 2026-06-20: tiered 30/90/180 + reclassify-terminali + esegui-fattuali), su standard docs-as-code (monthly-quick/quarterly-deep + archive-terminal) + ground-truth registry:
  - **Q1 cadence tiering SHIPPED #2909** (`e5ba7465`): 303 doc su cicli sub-mensili (289@14d + 14@7d = il treadmill) -> tier raise-only 30d reference / 90d spec-draft / 180d canon(core/adr/combat/SoT). Root-cause del treadmill RISOLTO. stale 172->101. 33 SoT frontmatter sincronizzati.
  - **Q3 residue factual-sync SHIPPED** (questo PR): 5 doc con frontmatter GIA' terminale ma registry out-of-sync -> registry allineato (3 `superseded`: ADR-2026-04-13-rules-engine-d20 [->kill-python ADR], pitch/vision [->00-SOURCE-OF-TRUTH], 2026-04-26-calibration-harness-policy [->CANONICAL-AI-PLAYTEST]; 2 `integrato`->`historical_ref`: incoming INTEGRATION_PLAN + TASKS_BREAKDOWN). Successori verificati esistenti. Chiude la parte incoming/pitch di TKT-STALE-B10-SUPERSEDE.
  - **Q2 SHIPPED (questo PR)**: 🔑 ground-truth ha raffinato "terminal-reclassify completo" -> ~meta' delle 100 stale NON erano record-terminali ma **canon/ADR/spec VIVI**, stale solo perche' Q1 aveva scopato i sub-30 lasciando i 30-90d. Reclassificare ADR vivi -> historical_ref = SBAGLIATO. Quindi 2 parti:
    - **(A) cadence-completion**: estesa la tiering Q1 (raise-only) a TUTTI i 30-90d -> 279 doc (190x 30->90, 58x 30->180, 18x 90->180, ...) + 46 SoT frontmatter. Canon/ADR/spec VIVI tornano in-window (live, non terminali). **stale 100->61.**
    - **(B) terminal-reclassify**: 25 doc terminali HIGH-CONFIDENCE -> `historical_ref` (frontmatter+registry): 7 web-v1 frontend (mockup/wireframe/styleguide superseduti dal pivot) + 3 idea-engine-web tutorials + flusso-richieste-idea-engine (server/app.js old) + 8 dead-process (web_pipeline / incoming_agent_streams+triage [AG-* archiviata] / telemetry_ingestion [mai costruito] / token-rotation [dead] / trait_rollout_plan [gate mai esistiti] / nebula-rollout + deploy-min [CI/render morti]) + 4 log datati (AI_AGENT_AUDIT_LOG / chatgpt_sync_status / tool_run_report / workflow_diff) + Frattura_pipeline_run + rollout_plan (schedule scaduto). **stale 61->36.**
  - **Q2 RESIDUO (LEFT, owner-gated) = ~36 stale**: (1) **DRIFTED-rewrite** (sistema esiste, doc da riscrivere NON archiviare): `ci/README` (CI attuale), `architecture/resistance-engine-gap` (re-frame su Node), `ops/{cloudflare-prod-deploy-guide [prod E' cloudflared], evo-tooling, observability, publishing_calendar, qa-window-checklist}`, `pipelines/{GOLDEN_PATH*, PIPELINE_*, BIOME_FEATURE_CHECKS, ci-*, gh-cli}`, `process/{bug-template, qa_reporting_schema, telemetry, clone-setup, demo-release, project-setup-todo, trait_data_reference, traits_checklist, incoming_triage}`, `guide/{CONTRIBUTING_SITE, structure_overview, INDEX, faq, INTEGRAZIONE_GUIDE, data-guidelines, templates/*}`, `public/sentience_sdk`; (2) **SoT source_of_truth:true** (10, owner verdict): COMMIT_STYLE, ADR-04-17-coop-scaling, ADR-04-21-campaign-save, ADR-04-21-meta-progression-prisma, ADR-05-10-sprint-q-lineage, planning/2026-05-14-phase-b-cutover, playtest/2026-05-14-phase-b-day-7, playtest/AGENT_DRIVEN_WORKFLOW, skiv/CANONICAL, skiv/LAUNCHER; (3) reports/\* SUPERSEDE (gia' TKT-STALE-B6). **Campagna stale 397->36** (-361). Lasciati = decisione owner (rewrite vs retire / SoT verdict).
  - **#2914 closeout (2026-06-21, master-dd verdetti via AskUserQuestion)**: **D+E keep+bump DONE** (PR #2923 `0a0857d3` -- 12 doc bump `last_verified` fm+registry; stale **36->24**). Verify-first (anti-pattern #8) ha REFUTATO il drift-guess: il tooling primario di ogni doc D/E e' LIVE (`game_cli.py`, `roll_pack.py`, `validate_dashboard.py`, `visual_regression.py`, `driveSync.gs`, `ExportModal.tsx`, `telemetry-export.yml`, trait-pipeline scripts) -> erano `minor stale paths` non DRIFTED. `ops/cloudflare-prod-deploy-guide` re-class DRIFTED->CURRENT (prod E' cloudflared, named-tunnel `evo-tactics-prod`, port 3334). **Group A STUDIED** (issue #2914 comment, 3-agent verify-first fan-out, NESSUN blind-retire): 9 doc = **4 clean-RETIRE** (`structure_overview`, `INDEX` [legacy nav superseded da docs/00-INDEX+README], `publishing_calendar`, `qa-window-checklist` [ticket #1204/#1205 misattribuiti]) + **5 SALVAGE-THEN-RETIRE** [`CONTRIBUTING_SITE`=Lighthouse/html-validate QA snippet se Pages-site mantenuto / `faq`=game_cli.py CLI subcommand-ref (piu' completo di CLAUDE.md) / `sentience_sdk`=**ACTIVE HAZARD** (endpoint `POST /traits/import` + `GET /sentience/tiers` MAI costruiti, advertizza vaporware a integrator; tier T0-T6 canon vivo in `README_SENTIENCE.md` + hook interocezione vivi in `active_effects.yaml`) -> caveat + fix inbound ref `sentience_rollout_plan` / `tooling_maintenance_log`=chore `unzip -o` in `report_incoming.sh` mai applicato -> BACKLOG / `resistance-engine-gap`=RESOLVED (`resistanceEngine.js` built+wired+tested, Python rules killed) tranne design-fork M6-#2 per-species `resistance_archetype` override+CI-guard (build-or-close)]. Retire/salvage execution = master-dd evaluating (owner-gated). **B+C REWRITE DONE** (master-dd: "continuiamo con B+C"): **B #2931 `94800406`** (6 doc CI/ops re-anchored su split `ci.yml` paths-filter->13-job->ci-gate + ops mission-console/observability) + **C #2933 `f258de2c`** (8 standards re-anchored su `data/core/species/*`+`species_catalog.json`+canon-enforcement + Urban_Flood_Missions RETIRED dead-one-off). 4-agent verify-first fan-out, claim load-bearing re-verificate da me (B1 false-report su 2 doc -> riscritti io; lesson ground-truth>agent-report). Metodo: `docs/guide/docs-governance-stale-lifecycle.md`.
  ...
  - **#2914 Group A EXECUTED (2026-06-21, master-dd verdetto via AskUserQuestion: RETIRE-tutti-9 + porta salvage)**: 9 doc A -> `historical_ref` (fm+registry; INDEX registry-only obsidian-fm). Salvage portato: `sentience_sdk` retire-banner anti-hazard + caveat `README_SENTIENCE` (SoT, body-only) + fix inbound ref `sentience_rollout_plan`; `faq` CLI-ref (alias `validate-ecosystem` + `investigate --destination -` + troubleshooting "Validator non trovato") -> `bug-template.md`; `CONTRIBUTING_SITE` retire-secco (Pages-site NON piu' mantenuto, master-dd). **stale 24->15.** 2 TODO salvage residui (sotto).
    ...
    - ✅ TKT-SALVAGE-A2 (O8) -- **GUARD DONE [#3080](https://github.com/MasterDD-L34D/Game/pull/3080)** (2026-06-30): CI-guard `tests/services/speciesArchetypeReferences.test.js` (ogni species `resistance_archetype` ∈ set canonico corazzato/bioelettrico/psionico/termico/adattivo; recursive collector; non-no-op; CI-wired test:api) + remap 5 retired-species (#3032) da `strutturale` (archetipo inesistente -> silent-fallback ad adattivo) a `adattivo` (band-neutral). **schema-field formalization DONE -> [PR #3147](https://github.com/MasterDD-L34D/Game/pull/3147) OPEN** (2026-07-01, forbidden-path -> master-dd manual merge): enum canonico `[adattivo,bioelettrico,corazzato,psionico,termico]` aggiunto a `species.schema.json` (additive/optional; chiude il workaround `additionalProperties:true`); dry-run 107/107 valori gia' canonici (0 rotture) + 3 test enum CI-wired `tests/api/contracts-species-archetype.test.js`; firma forbidden-path via AskUserQuestion (2-step) -- **MERGED `833416d8` 2026-07-02** (owner-authorized). **Design pass DONE -> [PR #3173](https://github.com/MasterDD-L34D/Game/pull/3173)** (2026-07-02, verdetto owner "tutte e 12"): 12 specie adattivo->reale (3 termico / 4 psionico / 4 corazzato / 1 bioelettrico), tutte combat-inerti (zero roster); 7 codex allineati (pattern #3087, il guard #3090 ne ha beccati 5 oltre il grep); orbital-ascendant DEFERITO (ambiguita' bioma-vs-keeper, decidere al rostering). **TKT-SALVAGE-A2 / O8 = ARCO CHIUSO.**
...
- **Batch-6 residue tickets** (reports; verdetti 3-agent fan-out read-only, successori SUPERSEDED verificati esistenti):
  - TKT-STALE-B6-SUPERSEDE -- 15 report SUPERSEDED (candidate `doc_status: superseded`, flip owner-gated come ~~TKT-STALE-B2-SUPERSEDE~~). Mappa successore:
    - **-> reconstruction-suite SPEC-A..Q** (canonical `docs/core/00-SOURCE-OF-TRUTH.md` v5 + `docs/planning/2026-06-06-evo-tactics-reconstruction-suite-index.md`): la design-reconciliation/synthesis layer 2026-04-27 = `5axes-ui-mapping-research`, `deep-analysis-residual-gaps-synthesis`, `identity-dimensions-reconciliation`, `identity-stack-reconciliation`, `skiv-portable-companion-research-summary` (-> SPEC-F crossbreed), `stato-arte-completo-vertical-slice` (self-declares supersede v3.7), `strategy-research-MASTER-synthesis`, `v3-vs-v3.2-reconciliation`, `v3.7-cross-pc-update-synthesis`, `vc-axes-reconciliation` (10).
    - **2026-04-26-deep-analysis-SYNTHESIS** -> `2026-04-27-deep-analysis-residual-gaps-synthesis` (P0 10/10 shipped; il successore e' a sua volta -> SPEC suite).
    - **2026-04-26-forme-naming-integration** -> `2026-04-27-stadio-phase-A-summary` (Q1 5-vs-10 stadi risolto = 10 stadi I-X eseguito).
    - **drift-audit series** (governance-illuminator ricorrente): `2026-05-18-drift-audit` -> `2026-05-25-drift-audit` -> `2026-06-08-drift-audit` (i 2 vecchi superseded dal piu' recente).
    - **2026-04-28-canonical-doc-consolidation-plan** -> ESEGUITO (Opzione B = `PILLAR-LIVE-STATUS.md` shipped + #2504 CLAUDE prune).
  - ~~TKT-STALE-B6-PILLAR-REFRESH~~ **CLOSED 2026-06-30** -- content/date refresh DONE 06-29; VALORI 6/6 **ri-ratificati master-dd** (close-out Tier-2) + P1 restore `def++` via PILLAR delta 06-30 (#3096 `a6e0c034`). Non piu' behind-cadence.
- **Batch-7 residue tickets** (planning sotto-batch 1 = cluster April-2026; verdetti 3-agent fan-out read-only + re-check verify-first, successori SUPERSEDED verificati esistenti):
  - TKT-STALE-B7-SPECIESYAML-DRIFTED -- 2 LIVING design-doc ancorati su `data/core/species.yaml` + `data/core/species_expansion.yaml` rimossi (#2271): `2026-04-25-creature-concept-catalog` (concept-catalog, extension/data anchors entrambi rimossi; trait/biome cross-ref OK), `2026-04-27-forme-10-stadi-naming-spec` (draft, §5 cross-dimension schema sorgente `species.yaml:66/:72` + `species_expansion`; Phase A mai shippata, `sub_stages`/`stage_index` count=0) -> re-anchor a `data/core/species/` + `species_catalog.json` o retire verso il layer SPEC-20-SPECIE / canon-enforcement (estende [[TKT-STALE-B2-SPECIESYAML]] + [[TKT-STALE-B4-SPECIESYAML]]).
  - TKT-STALE-B7-SUPERSEDE -- 4 plan/strategy SUPERSEDED dal layer Godot-pivot 2026-04-29 (candidate `doc_status: superseded`, flip owner-gated come ~~TKT-STALE-B2-SUPERSEDE~~ / ~~TKT-STALE-B6-SUPERSEDE~~). Mappa successore (verificati esistenti):
    - **2026-04-28-asset-sourcing-strategy** + **2026-04-28-godot-migration-strategy** (entrambi draft, phased web-first + Godot-Fase-2) -> reversati da `docs/adr/ADR-2026-04-29-pivot-godot-immediate.md` (active, SoT) + `docs/planning/2026-04-29-master-execution-plan-v3.md`.
    - **2026-04-28-master-execution-plan** (self-labeled "v2 DRAFT") -> `2026-04-29-master-execution-plan-v3.md` (active, SoT, lv 2026-06-06); +drift: port-source `services/rules/resolver.py` ucciso (ADR-2026-04-19).
    - **2026-04-29-ermes-cleanup-handoff** (active "next-session entrypoint", decision-tree A1/Sprint-G.2b/E7) -> droppato da `ADR-2026-04-29-pivot-godot-immediate.md` (title: drop Sprint G.2b + rubric); la parte PR-record (17 PR) e' accurata-as-history, ma l'autorita' forward e' morta.
  - TKT-STALE-B7-NEEDS-HUMAN -- owner judgment (3): `2026-04-25-skiv-octokit-webhooks-proposal` (npm-dep proposal "pending Master DD approval"; `apps/backend/routes/skiv.js` live = premessa valida; verdetto mai registrato -> approve/reject/withdraw, NON bump), `visual-research-2026-04-19` + `visual-roadmap-2026-04-19` (`doc_status: active` living visual Wave-8..13 roadmaps; ref + web stack `apps/play/` presenti = non strict-DRIFTED, ma la direzione web-visual-wave e' abbandonata dal Godot-pivot -> retire-vs-port-to-Godot-art-bible).
  - Nota: 25/27 CURRENT bumpati = record storici datati (handoff/closure/kickoff/audit/realized-plan) = candidate `doc_status: archived` (esenta dal ciclo stale; flip owner-gated). 2 senza `last_verified` frontmatter (`m13-phase-b` kickoff, `p4-mbti-playtest` kickoff) = registry-only bump.
- **Batch-8 residue tickets** (planning sotto-batch 2 = May-2026 + 2 undated; verdetti 3-agent fan-out read-only + re-check verify-first, successori SUPERSEDED verificati esistenti):
  - TKT-STALE-B8-DRIFTED-ECO -- `2026-05-13-ecosystem-research-solution-plan` (`source_of_truth: false`, LIVING 22-ticket TKT-ECO plan) prescrive ancora un Python->Node trophic-port citando `tools/py/{foodweb,trophic_roles,cross_events_validator}.py` come runtime path DOPO aver riconosciuto che ADR-2026-04-19 ha ucciso il Python rules-engine, + apply-target `data/core/species.yaml` (rimosso #2271) in TKT-ECO-A4 -> re-anchor runtime su `apps/backend/services/combat|ecosystem/` + species su `data/core/species/` (estende [[TKT-STALE-B7-SPECIESYAML-DRIFTED]]).
  - TKT-STALE-B8-SUPERSEDE -- 5 plan/spec SUPERSEDED (candidate `doc_status: superseded`, flip owner-gated). Mappa successore (verificati esistenti):
    - **2026-05-08-sprint-q-kickoff-coordination** + **2026-05-08-sprint-q-lineage-merge-etl-scoping** (draft, gated su "Phase B ACCEPTED" mai scattato) -> mating/lineage/offspring riorganizzati in SPEC-E della reconstruction-suite (`docs/planning/2026-06-06-evo-tactics-reconstruction-suite-index.md`).
    - **2026-05-09-status-effects-phase-a-audit** + **2026-05-09-status-effects-phase-a-design** (coppia analisi pre-impl) -> il same-day `2026-05-09-status-effects-phase-a-handoff.md` e' il record finale (cita audit+design, registra PR-A #2138 + PR-B #2139 shippate).
    - **codex-in-game-aliena-integration** (undated, Wave-9 codex MVP plan; targeta `data/core/species.yaml` rimosso nel build-script) -> design ALIENA canonicalizzato in `docs/design/evo-tactics-aliena-enforcement-lore.md` (SPEC-H) + codex SHIPPATO (`apps/play/src/codexPanel.js`, `data/codex/`, #2828-#2830).
  - TKT-STALE-B8-NEEDS-HUMAN -- owner judgment (3): `2026-05-08-master-dd-decisions-evidence-based` (`source_of_truth: true`, gia' re-verified 2026-06-06 con caveat-block -> bump last_verified owner-gated, NON auto), `2026-05-14-phase-b-cutover-canonical-execution` (`source_of_truth: true` runbook "esegui ORA": il cascade [archive `apps/play/src`->`apps/play.archive/`, force-push tag] NON e' mai stato eseguito + `apps/play/src` ancora live -> contenuto stale, owner retire/flip), `personas/player-personas-validation` (`doc_owner: governance` living validation framework; chiude con "promozione a `docs/core/18-PLAYER_PERSONAS.md` dopo review" mai avvenuta [file assente] -> canon-promotion owner-gated).
- **Batch-9 residue tickets** (cluster research+architecture+frontend+appendici+tutorials; verdetti Workflow classify + verify-first io stesso [il verify-stage agent rate-limited dal server -> main-thread re-check, che ha catturato 4 errori-classifier]):
  - TKT-STALE-B9-WEB-V1-FRONTEND -- 7 doc frontend draft = wireframe/mockup/styleguide/changelog del web-v1 UI superseduto dal pivot Godot: `frontend/styleguide` (cita `webapp/src/components` + `docs/test-interface/` rimossi), `frontend/mockups_evo` + `frontend/wireframes/evo/README` + `frontend/wireframes/evo/mockup_evo_tactics` + `frontend/wireframes/generatore` + `frontend/wireframes/homepage` (mockup/wireframe console web "riscrittura AngularJS", route web `/generatore` ecc.), `frontend/feature-updates` (changelog feature Canvas/Flow Shell web-v1) -> retire o port-to-Godot-art-bible (owner: design/UX). NB: `frontend/accessibility-deaf-visual-parity` = CURRENT (a11y cross-cutting WCAG, porta a Godot) -> bumpato.
  - TKT-STALE-B9-IDEA-ENGINE-WEB -- 4 doc che documentano l'idea-engine web-v1 UI rimossa: `architecture/flusso-richieste-idea-engine` (cita `webapp/` Vue + `server/app.js` + `server/storage.js` rimossi; backend reale = `apps/backend/index.js`), `tutorials/idea-engine` + `tutorials/idea-engine-feedback` (`docs/ideas/index.html` rimosso + porta stale `0.0.0.0:3333` -> ora 3334), `tutorials/dashboard-tour` (`npm --prefix webapp run dev` + `localhost:5173` + `docs/test-interface/` rimossi) -> re-frame su `apps/backend` + porta 3334 o archivia (owner).
  - TKT-STALE-B9-RULES-ENGINE -- `architecture/resistance-engine-gap` (`doc_status: active` living gap-doc il cui intero soggetto = `services/rules/{resolver,hydration,demo_cli}.py` ucciso ADR-2026-04-19; resistance ora in Node `apps/backend/services/combat/resistanceEngine.js`) -> re-frame su runtime Node o archivia (estende [[TKT-STALE-B3-RULES-ENGINE]]).
  - TKT-STALE-B9-NEEDS-HUMAN -- 5 research `source_of_truth: true` (owner-gated bump, NON auto): `2026-05-09-aggressive-h1-validation-oscillation`, `2026-05-09-aggressive-profile-calibration`, `2026-05-09-k3-fix-shipped-k4-audit`, `2026-05-09-k4-commit-window-implementation`, `2026-05-09-k4-stickiness-implementation` (calibration-record SoT-flagged; verdetto re-verify/archive = owner).
  - Nota minore (non-drift, bumpati): `architecture/i18n-strategy` (1 path stale `apps/dashboard/src/i18n` "(post-approval)" -> i18n shippato in `apps/play/src/i18n`), `tutorials/feedback-form` (2 bullet web-access "Webapp di playtest"/dashboard stale, ma form Notion + `tools/feedback/form_config.yaml` + pipeline live) -> fix inline 1-riga, NON residue-blocker. Research datati (gap-audit/od-022/evo-swarm/trait-orphan) citano `species.yaml`/`services/rules` come ref STORICI-veri alla loro data = CURRENT-as-record.
- **Batch-10 residue tickets** (cluster dir-mai-battute = museum+superpowers+governance+pitch+skiv+design+incoming+singletons; verdetti 3-agent fan-out read-only + verify-first io stesso [Agent-tool leggero NON throttled, vs Workflow 48-agent rate-limited in B9]):
  - TKT-STALE-B10-DRIFTED-WEBV1 -- 2 reference VIVI il cui infra autoritativo = web-v1 site-audit rimosso: `README_FULL_ARCHIVE` (cita workflow `deploy-test-interface.yml` rimosso + script `test:e2e`/`lint:lighthouse` assenti da package.json + `apps/dashboard` mock #1343 + `docs/ideas/`), `ci/README` (link-checker/Lighthouse/E2E Playwright + `test:e2e`/`lint:lighthouse` web-v1 site-audit, assenti) -> riscrivi vs CI attuale (`.github/workflows/`) + backend port 3334 o archivia (estende [[TKT-STALE-B2-CI-DRIFT]]).
  - TKT-STALE-B10-DRIFTED-MISC -- 3 reference VIVI a infra rimossa/non-costruita: `operativo/README-biodiversita-connessa` (input autoritativo `data/core/species.yaml` rimosso #2271; lo script `export_biodiversity_bundle.py` self-flagga legacy -> re-anchor `data/core/species/species_catalog.json`), `public/sentience_sdk` (API `GET /sentience/tiers` + `POST /traits/import` MAI costruite -- 0 hit nel backend; SDK-ref a infra inesistente -> build-or-retire owner), `governance/rollout_plan` (schedule 6-8 settimane finito 25-May-2026, governance ora strict-enforced -> archivia come dated-record completato).
  - TKT-STALE-B10-SUPERSEDE -- 3 doc terminali (candidate `doc_status` flip owner-gated): `pitch/evo_tactics_game_vision_reconstructed` (frontmatter GIA' `doc_status: superseded` + `superseded_by: docs/core/00-SOURCE-OF-TRUTH.md` [verificato esiste] MA registry doc_status out-of-sync -> **registry-sync a superseded** = lo esenta dal ciclo stale, NON e' un flip-decisione [autore ha gia' deciso]), `incoming/lavoro_da_classificare/INTEGRATION_PLAN` + `incoming/lavoro_da_classificare/TASKS_BREAKDOWN` (frontmatter `doc_status: integrato`, work landed = `data/external/evo/{species,traits}` esistono; 16/17 task done -> flip `archived` owner).
...
>
> - **I3 (durable crossbreed cooldown)** -- PR [#3101](https://github.com/MasterDD-L34D/Game/pull/3101) PREPARED then **CLOSED 2026-06-30**. Verify-first (2026-06-30): `apps/backend/services/skiv/companionStateStore.js` `hydrateAsync` was DEFINED but not called anywhere; `app.js:851` creates the store but no startup/route hydrate. **UPDATE 2026-07-01**: `hydrateAsync` IS now called **per-lineage** by `lineageExists()` before the import/promote overwrite guard ([#3135](https://github.com/MasterDD-L34D/Game/pull/3135)); what is still missing for I3 is a **startup/route BULK hydrate** so a cold Prisma process sees all lineages before the cooldown check (+ a FIFO-eviction edge on `crossbreed_history` cap 10). The in-memory cooldown on main stays (within-run correct; restart-reset moot since companion state resets with it).
...
- **TKT-P6-TRAIT-ORPHAN-DESIGN-B** (~2h): 14 B-defer traits design call. Categorization audit doc:
  ...
  - **ISTRUTTORIA 2026-06-22** (ground-truth re-scope, anti-pattern #19): [`docs/planning/2026-06-22-tkt-p6-trait-orphan-design-b-istruttoria.md`](docs/planning/2026-06-22-tkt-p6-trait-orphan-design-b-istruttoria.md). Audit 43gg stale -> 11 trait enumerabili (NON 14: conteggio-fonte incoerente). Dopo SPRINT_020 + status-engine extension (#1811/#1822) + evaluator-gate (`requires_target_status`/`requires_target_tag` ORA implementati): restano solo **3 design-call genuine** (`antenne_wideband` dedup / `mente_lucida` balance / `cervello_predittivo` tier-slot) + **1 bug-fix** (`sussurro_psichico` status key `disoriented` -> runtime `disorient`) + **7 sbloccati** ora A-class (cluster magnetico + `aura_glaciale` + `tela_appiccicosa` + `marchio_predatorio` + `biochip_memoria`: status/evaluator ORA wired, serve solo assegnazione-a-specie). Design-call resta master-dd; opzioni + evidenza nel doc.
