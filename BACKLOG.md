...
- **Track C (master-dd-gated, design call NON autonoma)**: C1 L'Impronta design spec **SCRITTA** (`review_needed`, `docs/planning/2026-06-22-aa01-cap11-limpronta-reconciliation-spec.md`) = CAP-11 biomeResolver core + CAP-13/13b/14/14b/15/15b inputs + CAP-12 telemetry canon-home. **3 design-call aperte master-dd**: (1) biomeResolver disposition [affinity / retire / defer], (2) imprint-beat shape [additive-separate-phase / discard-keep-UX / defer], (3) telemetry canon-home [standalone+Prisma-migration / fold / defer]. C2 code post-ratifica (sottrae i pezzi gia' mergiati in Track A; cap-11 = umbrella superset; CAP-06 gia' landed #2958). MUST-NOT-REGRESS: biome route-canon (no body-part), fasi `character_creation`+`world_setup` separate, device-authority (no host-token start), target Godot (web=ref).
