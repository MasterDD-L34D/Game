...
- **Remaining dir-batch**: **TUTTE le dir CURRENT-bumpabili sono state battute (B1-B10).** Le ~172 stale residue ora = (a) i ticket-residue owner-gated B1-B10 (DRIFTED/SUPERSEDED/NEEDS-HUMAN, NON auto-bumpabili by-design) + (b) possibile **re-stale treadmill** dei bump early-batch (B1 adr+core @ 2026-06-07 con `review_cycle_days` corto -> ri-scaduti by 2026-06-20). **NEXT = (1) verifica treadmill** (re-run i CURRENT B1-B6 ancora coerenti = re-bump O proponi `review_cycle_days` piu' lungo / archive owner-gated, NON treadmill infinito) **+ (2) smaltimento owner-gated** dei residue ticketati (SUPERSEDE->doc_status flip, NEEDS-HUMAN->master-dd, DRIFTED->fix-or-archive). Entrambi = checkpoint master-dd. (Ricalcolare col report.)
