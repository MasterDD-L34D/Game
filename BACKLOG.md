...
- **TUTTE le dir CURRENT-bumpabili battute (B1-B10).** Decisioni master-dd (AskUserQuestion 2026-06-20: tiered 30/90/180 + reclassify-terminali + esegui-fattuali), su standard docs-as-code (monthly-quick/quarterly-deep + archive-terminal) + ground-truth registry:
  ...
  - **NEXT (master-dd authorized) = Q2 terminal-reclassify COMPLETO** (judgment-batch dedicato): dated-record vivi (planning/research/reports/playtest/museum handoff/closure/log/audit ancora `active`/`draft`) -> `historical_ref` + draft shipped/morti -> `superseded` (li esenta dal ciclo; `archived`/`deprecated` NON sono statuti validi nell'enum -> usa historical_ref/superseded). **+ smaltimento owner-gated residue ticketati B1-B10** (NEEDS-HUMAN sot:true verdetti / DRIFTED fix-or-archive). Le ~100 stale residue = residue-ticketati B1-B10 + ADR/draft vecchi (target Q2). (Ricalcolare col report.)
