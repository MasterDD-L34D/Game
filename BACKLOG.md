...
- **TKT-DERIVED-ANALYSIS-REGEN** (fold in item-4 Family-2): `data/derived/analysis/trait_coverage_report.json` (+ bundle 8 artefatti + manifest sha256/commit-pin) frozen a commit `eb6b91d5` / 2025-12-09 (254 trait da index.json; CI usa trait_reference.json=173 e sovrascrive in-place -> gate-irrilevante, non stale puro = fonte diversa). Il generatore `scripts/generate_derived_analysis.py` NON gira sul repo attuale (bug stdout Windows su carattere non-ASCII + il sub-comando report_trait_coverage fallisce). Riparare generatore + rigenerare bundle + manifest = lavoro generator-determinism, da affrontare insieme a item-4 Family-2 catalog re-baseline (stesso macchinario derived/manifest, owner-gated).
...
- **TUTTE le dir CURRENT-bumpabili battute (B1-B10).** Decisioni master-dd (AskUserQuestion 2026-06-20: tiered 30/90/180 + reclassify-terminali + esegui-fattuali), su standard docs-as-code (monthly-quick/quarterly-deep + archive-terminal) + ground-truth registry:
  ...
  - **#2914 Group A EXECUTED (2026-06-21, master-dd verdetto via AskUserQuestion: RETIRE-tutti-9 + porta salvage)**: 9 doc A -> `historical_ref` (fm+registry; INDEX registry-only obsidian-fm). Salvage portato: `sentience_sdk` retire-banner anti-hazard + caveat `README_SENTIENCE` (SoT, body-only) + fix inbound ref `sentience_rollout_plan`; `faq` CLI-ref (alias `validate-ecosystem` + `investigate --destination -` + troubleshooting "Validator non trovato") -> `bug-template.md`; `CONTRIBUTING_SITE` retire-secco (Pages-site NON piu' mantenuto, master-dd). **stale 24->15.** 2 TODO salvage residui (sotto).
    ...
    - TKT-SALVAGE-A2 -- **BUILD ratificato (master-dd 2026-06-22, era build-or-close)**: costruire override per-specie `resistance_archetype` in species (via pipeline gen, MAI hand-edit catalog/derived) + CI-guard `speciesArchetypeReferences.test.js`. Sub-input design = mapping archetipo-per-specie (corazzato/bioelettrico/psionico/termico/adattivo; default `adattivo`, non-default = balance master-dd). Mechanical = schema field + guard + default; assegnazioni = pass design separato.
...
- **Batch-6 residue tickets** (reports; verdetti 3-agent fan-out read-only, successori SUPERSEDED verificati esistenti):
  ...
  - TKT-STALE-B6-PILLAR-REFRESH -- `PILLAR-LIVE-STATUS.md` = LIVING SOT master-dd-owned (`last_verified: 2026-06-01`, cycle 7gg -> ~19gg stale). 6 sprint landati dopo (item-1 17/17, lethal-wounds, dossier, SPEC-K) senza CONTRADDIRE il 6/6 verde, ma la board e' behind-cadence -> **refresh contenuto + re-verify owner/governance** (NON blind-bump su SoT; vedi [[project_pillar_status_sot]]).
