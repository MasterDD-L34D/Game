...
- [x] ~~**TKT-MUSEUM-SKIV-VOICES (P1 autonomous)**~~ → **✅ CHIUSO 2026-05-05** branch `feat/ennea-voice-palette`. Palette `data/core/narrative/ennea_voices/type_5.yaml` (Architetto, 21 lines × 7 beats) + `type_7.yaml` (Esploratore, 21 lines × 7 beats). Selector in `apps/backend/services/narrative/enneaVoice.js` (load + `getCandidateArchetypes` filtra triggered+supportati + `selectEnneaVoice` random pesato priority + `buildVoiceTelemetryEvent`). Export pass-through in `services/narrative/narrativeEngine.js`. Endpoint `GET /api/session/:id/voice?actor=X&beat=Y[&seed=N]` con Mulberry32 deterministico + emette session event `ennea_voice_type_used`. 14 unit test verde. AI baseline 383/383 preserved. Pre-req TKT-MUSEUM-ENNEA-WIRE soddisfatto (vcScoring già shippato per-round).
