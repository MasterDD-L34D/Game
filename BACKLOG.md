...
- **TUTTE le dir CURRENT-bumpabili battute (B1-B10).** Decisioni master-dd (AskUserQuestion 2026-06-20: tiered 30/90/180 + reclassify-terminali + esegui-fattuali), su standard docs-as-code (monthly-quick/quarterly-deep + archive-terminal) + ground-truth registry:
  ...
  - **#2914 Group A EXECUTED (2026-06-21, master-dd verdetto via AskUserQuestion: RETIRE-tutti-9 + porta salvage)**: 9 doc A -> `historical_ref` (fm+registry; INDEX registry-only obsidian-fm). Salvage portato: `sentience_sdk` retire-banner anti-hazard + caveat `README_SENTIENCE` (SoT, body-only) + fix inbound ref `sentience_rollout_plan`; `faq` CLI-ref (alias `validate-ecosystem` + `investigate --destination -` + troubleshooting "Validator non trovato") -> `bug-template.md`; `CONTRIBUTING_SITE` retire-secco (Pages-site NON piu' mantenuto, master-dd). **stale 24->15.** 2 TODO salvage residui (sotto).
    - TKT-SALVAGE-A1 -- chore `unzip -o` in `scripts/report_incoming.sh` (da `tooling_maintenance_log` ritirato): lo script usa `unzip -q`, aggiungere `-o` per evitare prompt interattivo su re-extract. Micro-fix non-bloccante.
    - TKT-SALVAGE-A2 -- design-fork M6-#2 (da `resistance-engine-gap` ritirato): decidere se serve override per-specie `resistance_archetype` in `data/core/species/*.yaml` + CI-guard `speciesArchetypeReferences.test.js`, OPPURE chiudere come job-mapping-canonical (engine gia' funziona via `JOB_ARCHETYPE` + fallback `adattivo`). master-dd/balance.
