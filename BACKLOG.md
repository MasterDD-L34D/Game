...
- **TUTTE le dir CURRENT-bumpabili battute (B1-B10).** Decisioni master-dd (AskUserQuestion 2026-06-20: tiered 30/90/180 + reclassify-terminali + esegui-fattuali), su standard docs-as-code (monthly-quick/quarterly-deep + archive-terminal) + ground-truth registry:
  ...
  - **TKT-SENT-\* sentience interocezione (programma multi-sprint, verdetti D1-D7 master-dd 2026-06-22 path massimalista)**: scope+verdetti+sequenza = `docs/planning/2026-06-21-sentience-traits-wiring-scope.md` sez.7-8. FATTO: Increment 1 producer #2932 `7053c830` (D1 minTier + D2 TIER_INTEROCEPTION_MAP infra + D4 perSpeciesOverride read-path, flag-OFF) + D6 engine#1 action-timing nocicezione #2936 `d38ffa92` + D6 engine#2 stamina-fatigue #2937 `3c6a0209` (flag-gated `STAMINA_FATIGUE_ENABLED` OFF). RESIDUO sequenziato: (1) D2 map ratify+N=40; (2) D4 populate pipeline; (3) **D3 enemy-wire** (UPDATE 06-22, era player-only); (4) D6 #3 encumbrance; (5) D7 flip incrementale (UPDATE 06-22, era defer). Ogni fase = PR + N=40 + sub-input design master-dd. Flag `SENTIENCE_INTEROCEPTION_GRANT_ENABLED` resta OFF (band-neutral).
