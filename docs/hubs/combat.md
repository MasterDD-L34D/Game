...
- `apps/backend/services/combat/reinforcementSpawner.js#tick` -- **ER6 overrun carry-over fork** (grilling 2026-06-30, [#3119](https://github.com/MasterDD-L34D/Game/pull/3119) `623860f5`): flag `REINFORCEMENT_OVERRUN_CARRYOVER_ENABLED` **default OFF**. Quando un tick NON spende tutto il bonus overrun (no walkable tile / cooldown / tier-below-min) l'unspent ACCUMULA su `state.overrun_carry` e si fonde nel bonus del tick successivo (vs l'as-built consume-once che lo scartava). `pool_exhausted` + `max_total` = terminale -> drop carry (no leak unbounded; Codex #3119 P2). Single-point in `tick` (entrambi i bridge consume-site beneficiano). OFF = byte-identical. N=40 band-SAFE **PROVISIONAL** (W5 non costruito). Evidence `docs/playtest/2026-06-30-er6-overrun-carryover-n40-evidence.md`.
- `apps/backend/services/combat/terrainReactions.js#chainLightningStrike` -- **D8 chain-lightning propagation** (aa01 CAP-07, #3082 + caps 3/2 [#3121](https://github.com/MasterDD-L34D/Game/pull/3121) `6b49023c`): BFS dell'electrified attraverso water tiles adiacenti (square-grid) + shock per-tile floored a 1 HP (mai killing blow; friendly-fire by design = telegraph). Flag `TERRAIN_CHAIN_LIGHTNING_ENABLED` **default OFF** (il single-tile electrify burst resta sempre attivo; SOLO la propagazione multi-tile e' gated). Caps PROPOSED `maxDepth 3 / shock 2` (grilling 2026-06-30, bump da 2/2; footprint sweep deterministico `tools/sim/d8-chain-footprint.js` = +1/+4/+12 occupanti line/plus/full-flood). **Flip DEFERRED** (master-dd 2026-07-01: non-band-neutral + inerte sul content attuale = nessun encounter electrified-terrain autorato). Evidence `docs/playtest/2026-06-30-d8-chain-lightning-cap-footprint.md`.

- `apps/backend/services/combat/{movementProfiles,moveCost,movementResolver}.js` + move-gate wire in `routes/session.js` (player + AI) e `services/abilityExecutor.js` (minion) — **move terrain-cost substrate** (spec `docs/superpowers/specs/2026-06-23-move-terrain-cost-substrate-design.md`). Flag `MOVE_TERRAIN_COST_ENABLED` **default OFF = band-neutral**: con flag ON il costo-AP del movimento = `ceil(cheapest terrain-weighted path)` (Dijkstra `moveCost`) sotto il `movement_profile` dell'unita' (esplicito > derive morphotype/form > medium); OFF = Manhattan invariato. `adattamento_volo` (graduato: g1 libera terreno normale, g2 dimezza hazard, g3 hazard-free) morde qui; il **grade e' per-creatura via il campo `unit.volo_grade`** (override clamp[1,3] letto da `evaluateVoloGrade`, fallback al base globale del registry poi 1; gate = presenza del trait; whitelisted in `normaliseUnit` come `speed`/`morale_mod`, quindi sopravvive al session-start e copre player+AI+minion registry-free). Terreno preso da `encounter.grid.terrain_features` → `session.grid` (assente = all-default = Manhattan). I range-check delle abilita' restano Manhattan (linea di tiro != costo cammino). Band-affecting quando ON → flip gated N=40 + master-dd (fase 4-5).
...
- `apps/backend/services/combat/membraneOsmotiche.js#applyTerrainHeal` — **membrane_osmotiche terrain-heal** (consumer del substrate, #3013). Fine-round in `sessionRoundBridge` (dopo il blocco filtri): un carrier vivo adiacente (4-neighbour) a un tile water/bog (`WATER_BOG_TERRAIN` = `acqua_profonda` canonico + `acqua`/`palude`/`fango` synonyms) cura 1 (cap `max_hp`). Legge `session.grid.terrain_features` (portate dal substrate). Band-neutral: nessun sim-carrier + nessuna griglia con tile water. Il tile-type "bog" canonico non e' ancora autorato (design-surface, master-dd ratifica il set al primo carrier).
- `apps/backend/services/combat/ecoSismico.js` — **eco_sismico tile-status primitive** (#3015, primo status keyed-per-tile, non per-unit). Store `grid.tile_statuses` = `[{x,y,status,source_id,expires_round}]`; `stampZonaRisonante` (seam del producer) / `zonaAt` (attiva se `round < expires_round`) / `applyZonaOnEnter` (entrare in una `zona_risonante` → `disorient`, source self-immune; consumer ai due move-site player+AI dopo il set-position) / `decayTileStatuses` (GC TTL fine-round, dopo il blocco pigmenti). Band-neutral: niente stampa una zona in combat (il producer = active ability e' un follow-up forbidden-path: nuovo `effect_type` + jobs re-baseline) + nessun sim-carrier.
