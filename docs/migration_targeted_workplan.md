...
5. **Promozione CI progressiva** (owner: dev-tooling)
   ...
   | validate_traits | patch/01C-tooling-ci-catalog | Verde        | Run #2–4 verdi (`logs/ci/validate_traits_run2.log`, `logs/ci/validate_traits_run3.log`, `logs/ci/validate_traits_run4.log`) archiviate in `logs/ci_patch-01C-tooling-ci-catalog.md`. | Tracking completato (3 run consecutive verdi, nessun fix coverage/index necessario).                                               |
   ...
   | validate-naming | patch/01C-tooling-ci-catalog | Consultivo   | Decisione condivisa e approvata in log `[01C-NAMING-CONSULTIVE-2026-04-26]` (`logs/agent_activity.md`).                                                                              | Confermato consultivo: 3 run `validate_traits` verdi raggiunte, enforcement rinviato finché la matrice core/derived non è stabile. |
   - Nota enforcement `validate-naming`:
     - Stato attuale: **consultivo** (trigger limitati a `push`/`workflow_dispatch`).
     - Decisione finale: mantenere consultivo (approvazione Master DD in `[01C-NAMING-CONSULTIVE-2026-04-26]`); riesame enforcement dopo stabilizzazione matrice core/derived (3 run `validate_traits` già verdi).
