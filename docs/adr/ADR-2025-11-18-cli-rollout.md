...
2. **Standardizzare la reportistica CLI** archiviando i log quotidiani generati da `scripts/cli_smoke.sh` (`logs/cli/smoke-YYYYMMDDTHHMMSSZ.log`, oppure varianti con `--label`/`--log-subdir`) e allegandoli alle checklist di progetto (`docs/checklist/`).
