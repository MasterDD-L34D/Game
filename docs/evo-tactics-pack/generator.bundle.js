...
// services/api/generatorClient.ts
...
function createContext(base) {
  ...
  const resolveDocHref = (relativePath) => {
    if (docsBase) {
      return resolveRelative(relativePath, docsBase);
    }
    return resolveRelative(relativePath, resolvedBase);
  };
  const resolvePackHref2 = (relativePath) => resolveRelative(relativePath, resolvedBase);
  return {
    resolvedBase,
    docsBase,
    catalogUrl: resolveDocHref("catalog_data.json"),
    resolveDocHref,
    resolvePackHref: resolvePackHref2
  };
}
async function fetchFromCandidates(options) {
  ...
  if (lastError) {
    throw lastError instanceof Error ? lastError : new Error(String(lastError ?? "Errore sconosciuto"));
  }
  return { data: null, url: null, fromFallback: false };
}
...
async function fetchCatalog(options = {}) {
  ...
  for (const base of candidates) {
    ...
    try {
      ...
      const data = await fetchJson(catalogUrl, fetchImpl, signal);
  ...
  if (manualFallback) {
    try {
      const data = await fetchJson(manualFallback, fetchImpl, signal);
  ...
  const message = "Impossibile caricare il catalogo del pack da alcuna sorgente candidata. Controlla la connessione o l'URL di base.";
...
async function fetchTraitRegistry(options) {
...
async function fetchTraitReference(options) {
...
async function fetchTraitGlossary(options) {
...
async function fetchHazardRegistry(options) {
...
async function fetchSpecies(options) {
...
// docs/evo-tactics-pack/pack-data.js
var PACK_PATH = "packs/evo_tactics_pack/";
...
function normalizeBase(value) {
  ...
  try {
    ...
    return ensureTrailingSlash2(absolute.toString());
  } catch (error) {
    console.warn("Impossibile normalizzare la base dati", value, error);
    return ensureTrailingSlash2(value);
  }
}
function detectRepoBase() {
  try {
    ...
    return ensureTrailingSlash2(`${origin}${basePath}`);
  } catch (error) {
...
function detectPackRootOverride() {
  try {
    ...
    const override = params.get("pack-root") || document.querySelector('meta[name="pack-root"]')?.getAttribute("content");
...
function detectGitHubRawRoot() {
  ...
  const owner = document.querySelector('meta[name="data-owner"]')?.getAttribute("content") || window.location.hostname.split(".")[0];
  const pathParts = window.location.pathname.split("/").filter(Boolean);
  const repo = document.querySelector('meta[name="data-repo"]')?.getAttribute("content") || pathParts[0] || "";
  const params = new URLSearchParams(window.location.search);
  const branch = params.get("ref") || document.querySelector('meta[name="data-branch"]')?.getAttribute("content") || DEFAULT_BRANCH;
  if (!owner || !repo) {
    return null;
  }
  return ensureTrailingSlash2(
...
function candidatePackRoots() {
  ...
  if (repoBase) {
    try {
      candidates.push(ensureTrailingSlash2(new URL(PACK_PATH, repoBase).toString()));
    } catch (error) {
  ...
  try {
    candidates.push(
      ensureTrailingSlash2(new URL(`../${PACK_PATH}`, window.location.href).toString())
    );
  } catch (error) {
  ...
  if (window.location.origin && window.location.origin !== "null") {
    ...
    candidates.push(ensureTrailingSlash2(`${origin}${PACK_PATH}`));
  }
  candidates.push(ensureTrailingSlash2(PACK_PATH));
  return Array.from(new Set(candidates.filter(Boolean)));
}
var PACK_ROOT_CANDIDATES = candidatePackRoots();
function resolveRelative2(relativePath, base) {
...
function getPackRootCandidates() {
  return [...PACK_ROOT_CANDIDATES];
}
async function loadCatalogFromCandidates(candidates = PACK_ROOT_CANDIDATES) {
  return fetchCatalog({ candidates });
}
async function manualLoadCatalog(options = {}) {
  const { candidates = PACK_ROOT_CANDIDATES } = options;
  return loadCatalogFromCandidates(candidates);
}
async function loadPackCatalog(options = {}) {
  const { candidates = PACK_ROOT_CANDIDATES } = options;
  return loadCatalogFromCandidates(candidates);
}
if (typeof window !== "undefined") {
  ...
  window.EvoPack.utils.ensureTrailingSlash = ensureTrailingSlash2;
  ...
  window.EvoPack.utils.resolveRelative = resolveRelative2;
...
// docs/evo-tactics-pack/ui/elements.ts
...
function resolveGeneratorElements(root = document) {
  return {
...
function resolveAnchorUi(root = document) {
  return {
...
// docs/evo-tactics-pack/views/anchor.js
var DEFAULT_LABEL = "";
...
function createAnchorNavigator(environment = {}) {
  ...
  const state2 = {
    ...
    descriptors: [],
    descriptorsById: /* @__PURE__ */ new Map(),
    sectionsById: /* @__PURE__ */ new Map(),
    minimaps: /* @__PURE__ */ new Map(),
    observer: null,
    scrollHandler: null,
    activeId: null,
    lastToggle: null,
    anchorHandlers: /* @__PURE__ */ new Map(),
    toggleHandlers: /* @__PURE__ */ new Map(),
    closerHandlers: /* @__PURE__ */ new Map(),
    overlayClickHandler: null,
    keydownHandler: null,
    hashHandler: null
  };
  function setUi(ui) {
    state2.ui.anchors = filterElements(toArray(ui?.anchors));
    state2.ui.panels = filterElements(toArray(ui?.panels));
    state2.ui.breadcrumbTargets = filterElements(toArray(ui?.breadcrumbTargets));
    state2.ui.minimapContainers = filterElements(toArray(ui?.minimapContainers));
    state2.ui.overlay = ui?.overlay instanceof HTMLElement ? ui.overlay : null;
    state2.ui.codexToggles = filterElements(toArray(ui?.codexToggles));
    state2.ui.codexClosers = filterElements(toArray(ui?.codexClosers));
  }
  function getPanelIdFromHash() {
    if (!state2.win) return null;
    const hash = state2.win.location?.hash ?? "";
    if (!hash) return null;
    const id = hash.replace(/^#/, "");
    if (!id) return null;
    const element = state2.doc?.getElementById(id);
    if (!element) return null;
    return element.dataset?.panel ?? null;
  }
  function updateBreadcrumbs(sectionId) {
    const descriptor = state2.descriptorsById.get(sectionId);
    const label = descriptor?.label ?? DEFAULT_LABEL;
    state2.ui.breadcrumbTargets.forEach((target) => {
      target.textContent = label;
      target.dataset.activeAnchor = sectionId ?? "";
    });
  }
  function updateMinimapState() {
    state2.minimaps.forEach((registry) => {
      registry.forEach(({ item, progress }, id) => {
        const descriptor = state2.descriptorsById.get(id);
        if (!descriptor) return;
        const ratio = Math.max(0, Math.min(1, descriptor.progress ?? 0));
        progress.style.setProperty("--progress", `${Math.round(ratio * 100)}%`);
        if (state2.activeId === id) {
  ...
  function setActiveSection(sectionId, { updateHash = false, silent = false } = {}) {
    if (!sectionId || !state2.descriptorsById.has(sectionId)) {
      return false;
    }
    const previous = state2.activeId;
    state2.activeId = sectionId;
    if (previous !== sectionId || !silent) {
      state2.descriptors.forEach((descriptor) => {
    ...
    if (updateHash) {
      const panel = state2.sectionsById.get(sectionId)?.element;
      if (panel?.id && state2.win?.history?.replaceState) {
        state2.win.history.replaceState(null, "", `#${panel.id}`);
      }
    }
    return true;
  }
  ...
  function scrollToPanel(panelId, { smooth = true } = {}) {
    ...
    const entry = state2.sectionsById.get(panelId);
    ...
    if (typeof state2.win?.requestAnimationFrame === "function") {
      state2.win.requestAnimationFrame(() => focusPanel(panel));
    } else {
      focusPanel(panel);
    }
    return true;
  ...
  function computeActiveSection() {
    if (!state2.descriptors.length) return null;
    const visible = state2.descriptors.filter((descriptor) => descriptor.isIntersecting);
    if (visible.length) {
    ...
    let minTop = Infinity;
    state2.descriptors.forEach((descriptor) => {
      const section = state2.sectionsById.get(descriptor.id);
    ...
    return state2.descriptors[state2.descriptors.length - 1]?.id ?? null;
  }
  ...
  function cleanupScrollFallback() {
    if (!state2.scrollHandler || !state2.win) {
      return;
    }
    state2.win.removeEventListener("scroll", state2.scrollHandler);
    state2.win.removeEventListener("resize", state2.scrollHandler);
    state2.scrollHandler = null;
  }
  function createScrollFallback() {
    if (!state2.win) return;
    state2.scrollHandler = () => {
      const viewportHeight = state2.win.innerHeight || state2.doc?.documentElement?.clientHeight || 1;
      state2.descriptors.forEach((descriptor) => {
        const section = state2.sectionsById.get(descriptor.id);
        const element = section?.element;
        if (!element) return;
    ...
    state2.win.addEventListener("scroll", state2.scrollHandler, { passive: true });
    state2.win.addEventListener("resize", state2.scrollHandler);
    state2.scrollHandler();
  }
  function createAnchorObserver() {
    ...
    if (!state2.sectionsById.size) return;
    const IntersectionObserverCtor = state2.win && state2.win.IntersectionObserver || globalThis.IntersectionObserver;
    ...
    state2.observer = new IntersectionObserverCtor(handleAnchorObserver, {
      rootMargin: "-40% 0px -40% 0px",
      threshold: thresholds
    });
    state2.sectionsById.forEach(({ element }) => {
      if (element) {
        state2.observer.observe(element);
      }
    });
  }
  function setupMinimaps() {
    state2.minimaps.clear();
    if (!state2.doc) return;
    state2.ui.minimapContainers.forEach((container) => {
      ...
      state2.descriptors.forEach((descriptor) => {
        ...
        const progress = state2.doc.createElement("span");
      ...
      state2.minimaps.set(container, registry);
    });
    updateMinimapState();
  }
  function closeOverlay({ skipFocus = false } = {}) {
    const overlay = state2.ui.overlay;
    if (!overlay || overlay.hidden) {
      state2.lastToggle = skipFocus ? state2.lastToggle : null;
      return;
    }
    overlay.hidden = true;
    overlay.setAttribute("aria-hidden", "true");
    state2.doc?.body?.classList.remove("codex-open");
    if (!skipFocus && state2.lastToggle && typeof state2.lastToggle.focus === "function") {
      state2.lastToggle.focus();
    }
    state2.lastToggle = null;
  }
  function openOverlay() {
    const overlay = state2.ui.overlay;
    if (!overlay) return;
    if (!state2.minimaps.size) {
      setupMinimaps();
    }
    overlay.hidden = false;
    overlay.setAttribute("aria-hidden", "false");
    state2.doc?.body?.classList.add("codex-open");
    updateMinimapState();
  ...
  function setupOverlayControls() {
    ...
    state2.ui.codexToggles.forEach((button) => {
      const handler = () => {
        state2.lastToggle = button;
        openOverlay();
      };
      button.addEventListener("click", handler);
      state2.toggleHandlers.set(button, handler);
    });
    state2.ui.codexClosers.forEach((button) => {
      const handler = () => {
        closeOverlay();
      };
      button.addEventListener("click", handler);
      state2.closerHandlers.set(button, handler);
    });
    state2.overlayClickHandler = (event) => {
      if (event.target === overlay) {
        closeOverlay();
      }
    };
    overlay.addEventListener("click", state2.overlayClickHandler);
    if (state2.doc) {
      state2.keydownHandler = (event) => {
        if (event.key === "Escape" && state2.doc.body?.classList.contains("codex-open")) {
          closeOverlay();
        }
      };
      state2.doc.addEventListener("keydown", state2.keydownHandler);
    }
  }
  function attachAnchorHandlers() {
    state2.anchorHandlers.forEach((handler, anchor) => {
      anchor.removeEventListener("click", handler);
    });
    state2.anchorHandlers.clear();
    state2.descriptors.forEach((descriptor) => {
      const handler = (event) => {
      ...
      state2.anchorHandlers.set(descriptor.anchor, handler);
    });
  }
  function attachHashHandler() {
    if (!state2.win) return;
    if (state2.hashHandler) {
      state2.win.removeEventListener("hashchange", state2.hashHandler);
    }
    state2.hashHandler = () => {
    ...
    state2.win.addEventListener("hashchange", state2.hashHandler);
  }
  function detachOverlayControls() {
    state2.ui.codexToggles.forEach((button) => {
      const handler = state2.toggleHandlers.get(button);
      if (handler) {
        button.removeEventListener("click", handler);
      }
    });
    state2.toggleHandlers.clear();
    state2.ui.codexClosers.forEach((button) => {
      const handler = state2.closerHandlers.get(button);
      if (handler) {
        button.removeEventListener("click", handler);
      }
    });
    state2.closerHandlers.clear();
    if (state2.overlayClickHandler && state2.ui.overlay) {
      state2.ui.overlay.removeEventListener("click", state2.overlayClickHandler);
      state2.overlayClickHandler = null;
    }
    if (state2.keydownHandler && state2.doc) {
      state2.doc.removeEventListener("keydown", state2.keydownHandler);
      state2.keydownHandler = null;
    }
  }
  function detachHashHandler() {
    if (state2.hashHandler && state2.win) {
      state2.win.removeEventListener("hashchange", state2.hashHandler);
      state2.hashHandler = null;
    }
  }
  function destroy() {
    cleanupScrollFallback();
    ...
    closeOverlay({ skipFocus: true });
    state2.descriptors = [];
    state2.descriptorsById.clear();
    state2.sectionsById.clear();
    state2.minimaps.clear();
    state2.activeId = null;
    state2.lastToggle = null;
    setUi({});
  }
  function init(ui) {
    destroy();
    setUi(ui);
    if (!state2.ui.anchors.length || !state2.ui.panels.length) {
      return { activeId: null };
    }
    state2.sectionsById = /* @__PURE__ */ new Map();
    state2.ui.panels.forEach((panel) => {
      const panelId = panel.dataset.panel;
      if (!panelId) return;
      state2.sectionsById.set(panelId, { element: panel });
      if (!panel.hasAttribute("tabindex")) {
        panel.setAttribute("tabindex", "-1");
      }
    });
    state2.descriptors = state2.ui.anchors.map((anchor) => {
      const id = anchor.dataset.anchorTarget;
      if (!id || !state2.sectionsById.has(id)) return null;
    ...
    state2.descriptorsById = new Map(
      state2.descriptors.map((descriptor) => [descriptor.id, descriptor])
    );
    if (!state2.descriptors.length) {
      return { activeId: null };
    }
    ...
    if (initialId && state2.descriptorsById.has(initialId)) {
      setActiveSection(initialId, { silent: true });
    } else {
      setActiveSection(state2.descriptors[0].id, { silent: true });
    }
    return { activeId: state2.activeId };
  }
  return {
    init,
    setActiveSection,
    ...
    openOverlay,
    closeOverlay,
    hasSection: (sectionId) => state2.descriptorsById.has(sectionId),
    getActiveSection: () => state2.activeId
  };
}

// docs/evo-tactics-pack/state/session.ts
...
function createComposerState() {
  return {
    constraints: {
      minSynergy: 45,
      preferredRoles: /* @__PURE__ */ new Set()
    },
    combinedPresets: [],
    suggestions: [],
    metrics: {
...
function createSessionState(options = {}) {
  ...
  return {
    ...
    traitReference: null,
    traitGlossary: null,
    traitsIndex: /* @__PURE__ */ new Map(),
    traitDetailsIndex: /* @__PURE__ */ new Map(),
    traitGlossaryIndex: /* @__PURE__ */ new Map(),
    hazardRegistry: null,
    hazardsIndex: /* @__PURE__ */ new Map(),
    activityLog: [],
    activityFilters: {
      query: "",
    ...
    activityTagCounts: /* @__PURE__ */ new Map(),
    metrics: {
      averageRollIntervalMs: null,
      rerollCount: 0,
      uniqueSpecies: 0,
      filterProfileReuses: 0,
      roleUsage: /* @__PURE__ */ new Map()
    },
    ...
    cardState: {
      ...
      squad: /* @__PURE__ */ new Map(),
      comparison: /* @__PURE__ */ new Map(),
      locks: {
        biomes: /* @__PURE__ */ new Set(),
...
// docs/evo-tactics-pack/utils/ids.ts
function randomId(prefix = "synt") {
  const suffix = Math.random().toString(36).slice(2, 8);
  return `${prefix}-${suffix}`;
}

// docs/evo-tactics-pack/utils/normalizers.ts
var ROLE_FLAGS_DEFAULT = ["apex", "keystone", "bridge", "threat", "event"];
var CLIMATE_HINTS = [
...
function extractRequiredRoles(filters = {}, roleFlags = ROLE_FLAGS_DEFAULT) {
...
function inferMinSize(filters = {}, roleFlags = ROLE_FLAGS_DEFAULT) {
...
function buildGenerationConstraints(filters = {}, options = {}) {
...
function createTagEntry(tag, generator = randomId) {
  if (tag && typeof tag === "object") {
    const labelCandidate = tag.label ?? tag.name ?? tag.title ?? tag.text ?? tag.value ?? tag.id ?? "";
    const label2 = String(labelCandidate || "").trim();
    if (!label2) {
      return null;
    }
    const idCandidate = tag.id ?? tag.value ?? normaliseTagId(label2);
    const id2 = normaliseTagId(idCandidate || label2) || normaliseTagId(label2) || generator("tag");
    return { id: id2 || generator("tag"), label: label2 };
  }
...
// docs/evo-tactics-pack/utils/serializers.ts
...
function serialiseActivityLogEntry(entry) {
  ...
  const tags = Array.isArray(entry.tags) ? entry.tags.map((tag) => {
    if (!tag) return null;
    if (typeof tag === "object") {
      const id = tag.id ?? tag.value ?? null;
      const label2 = tag.label ?? tag.name ?? tag.value ?? tag.id ?? null;
      if (!id && !label2) return null;
      return {
        id: id ?? (label2 ? normaliseTagId(label2) || null : null),
        label: label2 ?? (id ? String(id) : "")
      };
    }
    const label = String(tag ?? "").trim();
...
`;
  ...
  const rows = entries.map((entry) => {
    ...
    const metadata = entry.metadata === null || entry.metadata === void 0 ? "" : typeof entry.metadata === "string" ? entry.metadata : JSON.stringify(entry.metadata);
    const record = {
      id: entry.id ?? "",
...
function toYAML(value, indent = 0) {
  ...
  if (Array.isArray(value)) {
    ...
    return value.map((item) => `${space}- ${toYAML(item, indent + 1).replace(/^\s*/, "")}`).join("\n");
  }
...
// docs/evo-tactics-pack/services/storage.ts
var STORAGE_KEYS = {
...
function detectLocalStorage(windowRef, logger) {
  ...
  try {
    ...
    logger?.warn?.("LocalStorage non disponibile, verr\xE0 utilizzato il fallback in-memory", error);
    return null;
  }
}
function createStorageService(options = {}) {
  ...
  let activeStorage = overrideStorage ?? detectLocalStorage(windowRef, logger) ?? memoryStorage;
  let persistent = activeStorage !== memoryStorage;
  function withStorage(action, fallbackValue) {
    try {
  ...
  function readRaw(key) {
    return withStorage((driver) => driver.getItem(key) ?? null, null);
  }
  ...
  return {
    ...
    loadActivityLog() {
      const data = loadJson(STORAGE_KEYS.activityLog, []);
    ...
    loadFilterProfiles() {
      const data = loadJson(STORAGE_KEYS.filterProfiles, []);
      return Array.isArray(data) ? data : [];
    },
    ...
    loadPreferences() {
      const data = loadJson(STORAGE_KEYS.preferences, null);
      if (!data || typeof data !== "object") {
        return null;
      }
      const result = {
        audioMuted: typeof data.audioMuted === "boolean" ? data.audioMuted : void 0,
        volume: typeof data.volume === "number" ? data.volume : void 0
      };
      if (result.audioMuted === void 0 && result.volume === void 0) {
        return null;
      }
      return result;
    },
    ...
    loadPinnedState() {
      const data = loadJson(STORAGE_KEYS.pinned, []);
      return Array.isArray(data) ? data : [];
    },
    savePinnedState(payload) {
      saveJson(STORAGE_KEYS.pinned, Array.isArray(payload) ? payload : []);
    },
    loadLocks() {
      const data = loadJson(STORAGE_KEYS.locks, { biomes: [], species: [] });
    ...
    loadHistory() {
      const data = loadJson(STORAGE_KEYS.history, []);
      return Array.isArray(data) ? data : [];
    },
    saveHistory(entries) {
...
// docs/evo-tactics-pack/services/audio.ts
var RARE_CUE_SOURCE = "data:audio/wav;base64,";
var DEFAULT_AUDIO_CUES = {
  rare: RARE_CUE_SOURCE
};
function clampVolume(value, fallback = 0.5) {
...
function defaultCreateAudioElement(src, initialVolume, logger) {
...
function createAudioCueFactory(options = {}) {
  const { cues = {}, createAudioElement, logger, initialVolume = 0.5 } = options;
  const cache = /* @__PURE__ */ new Map();
  const factory = createAudioElement ?? ((src) => defaultCreateAudioElement(src, initialVolume, logger));
  return {
    ensure(key) {
      if (!key) return null;
      if (cache.has(key)) {
...
function createAudioService(options) {
  ...
  const controlElements = {
    controls: elements2.controls ?? null,
    mute: elements2.mute ?? null,
    volume: elements2.volume ?? null
  };
  function persist() {
    savePreferences?.({
  ...
  function applyPreferences() {
    ...
    if (typeof muteButton?.textContent === "string") {
      muteButton.textContent = muted ? "\u{1F507} Audio disattivato" : "\u{1F508} Audio attivo";
    }
  ...
  function setupControls() {
    ...
    if (volumeInput?.addEventListener) {
      ...
      volumeInput.addEventListener("input", (event) => {
        const target = event.target?.value ?? volumeInput.value;
        handleVolume(target);
      });
...
// services/export/dossier.ts
function ensureFetcher(fetchImpl) {
  if (typeof fetchImpl === "function") {
    return fetchImpl;
  }
...
async function loadDossierTemplate(path, options = {}) {
...
function flattenSpeciesBuckets(buckets = {}) {
  ...
  Object.values(buckets).filter((list) => Array.isArray(list)).forEach((list) => {
    list.forEach((entry) => {
      if (!entry || typeof entry !== "object") return;
      const key = entry.id || entry.display_name || entry.displayName || entry.speciesId || null;
...
function formatSpotlightLine(entry) {
  ...
  const tags = [biomeLabel, tierLabel, synthLabel].filter(Boolean).join(" \xB7 ");
  return tags ? `- ${name} (${tags})` : `- ${name}`;
}
function formatSpeciesFallbackLine(species, helpers) {
  ...
  const biomeLabel = helpers.findBiomeLabelById?.(biomeCode) ?? biomeCode;
  const roleCode = species.role_trofico ?? species.role ?? null;
  const resolvedRole = typeof roleCode === "string" ? helpers.roleLabels[roleCode] ?? helpers.titleCase(roleCode.replace(/_/g, " ")) : null;
  const tags = [biomeLabel, resolvedRole].filter(Boolean).join(" \xB7 ");
  return tags ? `- ${name} (${tags})` : `- ${name}`;
}
function formatSeedSummary(seed, helpers) {
  if (!seed) return null;
  ...
  const segments = [label, biomeLabel, threat, synth].filter(Boolean).join(" \xB7 ");
  return segments ? `- ${segments}` : null;
}
function generateDossierDocument(context, helpers) {
  ...
  const generatedAt = context.generatedAt ? context.generatedAt instanceof Date ? context.generatedAt : new Date(context.generatedAt) : /* @__PURE__ */ new Date();
  const locale = "it-IT";
  const generatedLabel = generatedAt.toLocaleString(locale, { hour12: false });
  setSlotText("title", `${context.ecosystemLabel} \xB7 Dossier`);
  setSlotText("heading", context.ecosystemLabel);
  setSlotText(
    "badge",
    helpers.presetLabel ? `Preset \xB7 ${helpers.presetLabel}` : "Ecosystem dossier"
  );
  ...
  setSlotText("summary", summaryParts.join(" \xB7 "));
  ...
  if (activityContainer) {
    ...
    context.activityEntries.slice(0, 12).forEach((entry) => {
      ...
      item.textContent = text || "Evento attivit\xE0";
  ...
  if (biomeContainer) {
    ...
    context.biomes.slice(0, 6).forEach((biome) => {
      ...
      heading.textContent = headingParts.filter(Boolean).join(" \xB7 ");
  ...
  if (speciesContainer) {
    ...
    flattenSpeciesBuckets(context.speciesBuckets).slice(0, 12).forEach((entry) => {
      const record = entry;
      const item = doc.createElement("li");
      item.className = "dossier__list-item";
      const heading = doc.createElement("h3");
      ...
      meta.textContent = metaParts.length ? metaParts.join(" \xB7 ") : "Dati sintetici";
      item.append(heading, meta);
      const tags = Array.isArray(record.functional_tags) ? record.functional_tags.slice(0, 8) : [];
      if (tags.length) {
  ...
  if (seedsContainer) {
    ...
    context.seeds.slice(0, 10).forEach((seedEntry) => {
      ...
      heading.textContent = headingParts.join(" \xB7 ");
      const meta = doc.createElement("p");
...
async function generateDossierHtml(context, helpers) {
...
async function generateDossierPdfBlob(context, options) {
  ...
  const worker = options.html2pdf().set({
    margin: 10,
...
function buildPressKitMarkdown(context, helpers) {
  ...
  if (highlightBiome) {
    const label = highlightBiome.label ?? highlightBiome.id ?? "Bioma";
    lines.push(`- Bioma in evidenza: ${label}`);
  }
  lines.push("");
  const pinnedLines = Array.isArray(context.pinnedEntries) ? context.pinnedEntries.map((entry) => formatSpotlightLine(entry)).filter(Boolean).slice(0, 3) : [];
  let spotlightLines = pinnedLines;
  if (!spotlightLines.length) {
    spotlightLines = flattenSpeciesBuckets(context.speciesBuckets).map((species) => formatSpeciesFallbackLine(species, helpers)).filter(Boolean).slice(0, 3);
  }
  if (spotlightLines.length) {
  ...
  const seedLines = Array.isArray(context.seeds) ? context.seeds.map((seed) => formatSeedSummary(seed, helpers)).filter(Boolean).slice(0, 4) : [];
  if (seedLines.length) {
...
async function generatePresetFileContents(preset, context, options) {
  const files = [];
  let cachedTemplate;
  let cachedHtml;
  let cachedPdf;
  const ensureTemplate = async () => {
    if (cachedTemplate !== void 0) return cachedTemplate;
    cachedTemplate = await options.templateLoader();
    return cachedTemplate;
  };
  const ensureHtml = async () => {
    if (cachedHtml !== void 0) return cachedHtml;
  ...
  const ensurePdf = async () => {
    if (cachedPdf !== void 0) return cachedPdf;
    if (!options.html2pdf) {
      cachedPdf = null;
      return cachedPdf;
    }
  ...
  for (const file of preset.files) {
    const filename = typeof file.filename === "function" ? file.filename(context.slug) : file.filename;
    try {
      switch (file.builder) {
        case "ecosystem-json":
          files.push({
...
// docs/evo-tactics-pack/views/parameters.js
var parameters_exports = {};
__export(parameters_exports, {
  render: () => render,
  setup: () => setup
});
function setup(state2, elements2, deps = {}) {
  deps.setupFilterChangeHandlers?.(state2, elements2);
  deps.attachProfileHandlers?.(state2, elements2);
  deps.setupExportControls?.(state2, elements2);
}
function render(state2, elements2, deps = {}) {
  const {
    context = {},
    renderProfileSlots: renderProfileSlots2,
    renderExportManifest: renderExportManifest2,
    renderKpiSidebar: renderKpiSidebar2,
    currentFilters: currentFilters2
  } = deps;
  const refreshTargets = Array.isArray(context.refresh) ? new Set(context.refresh) : /* @__PURE__ */ new Set(["profiles", "export", "kpi"]);
  if (refreshTargets.has("profiles") && typeof renderProfileSlots2 === "function") {
    renderProfileSlots2(state2, elements2);
  }
  if (refreshTargets.has("export") && typeof renderExportManifest2 === "function") {
    const filters = context.filters ?? (typeof context.getFilters === "function" ? context.getFilters() : void 0) ?? (typeof currentFilters2 === "function" ? currentFilters2() : void 0);
    renderExportManifest2(filters);
  }
  if (refreshTargets.has("kpi") && typeof renderKpiSidebar2 === "function") {
    renderKpiSidebar2(state2, elements2);
  }
}

// docs/evo-tactics-pack/views/traits.js
var traits_exports = {};
...
function render2(state2, elements2, deps = {}) {
  deps.renderTraitExpansions?.(state2, elements2);
}

// docs/evo-tactics-pack/views/biomes.js
var biomes_exports = {};
__export(biomes_exports, {
  render: () => render3,
  setup: () => setup3
});
function setup3(state2, elements2, deps = {}) {
  deps.attachComparisonHandlers?.(state2, elements2);
}
function render3(state2, elements2, deps = {}) {
  const { context = {}, renderBiomes: renderBiomes2, currentFilters: currentFilters2 } = deps;
  const filters = context.filters ?? (typeof currentFilters2 === "function" ? currentFilters2() : void 0);
  renderBiomes2?.(filters);
}

// docs/evo-tactics-pack/views/seeds.js
var seeds_exports = {};
...
function render4(state2, elements2, deps = {}) {
  deps.renderSeeds?.(state2, elements2);
}

// docs/evo-tactics-pack/views/composer.js
var composer_exports = {};
__export(composer_exports, {
  render: () => render5,
  setup: () => setup5
});
function setup5(state2, elements2, deps = {}) {
  deps.attachComposerHandlers?.(state2, elements2);
}
function render5(state2, elements2, deps = {}) {
  deps.renderComposerPanel?.(state2, elements2);
}

// docs/evo-tactics-pack/views/insights.js
var insights_exports = {};
__export(insights_exports, {
  render: () => render6,
  setup: () => setup6
});
function setup6(state2, elements2, deps = {}) {
  deps.attachInsightHandlers?.(state2, elements2);
}
function render6(state2, elements2, deps = {}) {
  const { context = {}, renderContextualInsights: renderContextualInsights2, getRecommendations } = deps;
  const recommendations = context.recommendations ?? (typeof getRecommendations === "function" ? getRecommendations() : []);
  renderContextualInsights2?.(recommendations);
}

// docs/evo-tactics-pack/views/activity.js
function setup7(state2, elements2, deps = {}) {
  const triggerRender = () => {
    if (typeof deps.render === "function") {
      deps.render(state2, elements2, deps);
    } else {
      render7(state2, elements2, deps);
    }
  };
  if (elements2.activitySearch) {
    elements2.activitySearch.addEventListener("input", (event) => {
      state2.activityFilters = state2.activityFilters ?? {};
      state2.activityFilters.query = String(event.target.value ?? "");
      triggerRender();
    });
  }
  if (elements2.activityTagFilter && typeof deps.getSelectedValues === "function") {
    elements2.activityTagFilter.addEventListener("change", () => {
      const values = new Set(deps.getSelectedValues(elements2.activityTagFilter));
      state2.activityFilters = state2.activityFilters ?? {};
      state2.activityFilters.tags = values;
      triggerRender();
    });
  }
  if (elements2.activityPinnedOnly) {
    elements2.activityPinnedOnly.addEventListener("change", (event) => {
      state2.activityFilters = state2.activityFilters ?? {};
      state2.activityFilters.pinnedOnly = Boolean(event.target.checked);
      triggerRender();
    });
  }
  if (Array.isArray(elements2.activityToneToggles)) {
    elements2.activityToneToggles.forEach((toggle) => {
      if (!(toggle instanceof HTMLInputElement)) return;
      toggle.addEventListener("change", (event) => {
        const tone = event.target.value || event.target.dataset.activityTone;
        if (!tone) return;
        state2.activityFilters = state2.activityFilters ?? {};
        if (!(state2.activityFilters.tones instanceof Set)) {
          state2.activityFilters.tones = new Set(deps.defaultTones ?? []);
        }
        if (event.target.checked) {
          state2.activityFilters.tones.add(tone);
        } else {
          state2.activityFilters.tones.delete(tone);
        }
        triggerRender();
      });
    });
  }
  if (elements2.activityReset && typeof deps.resetFilters === "function") {
    elements2.activityReset.addEventListener("click", (event) => {
      event.preventDefault();
      deps.resetFilters();
    });
  }
  if (elements2.logList && typeof deps.togglePin === "function") {
    elements2.logList.addEventListener("click", (event) => {
  ...
  syncControls(state2, elements2, deps);
}
function render7(state2, elements2, deps = {}) {
  const list = elements2.logList;
  const empty = elements2.logEmpty;
  if (!list) return;
  list.innerHTML = "";
  const entries = Array.isArray(state2.activityLog) ? state2.activityLog : [];
  const filters = state2.activityFilters ?? {};
  ...
  const tagSet = filters.tags instanceof Set ? filters.tags : /* @__PURE__ */ new Set();
  const query2 = String(filters.query ?? "").trim().toLowerCase();
  const pinnedOnly = Boolean(filters.pinnedOnly);
  const buildHaystack = typeof deps.buildActivityHaystack === "function" ? deps.buildActivityHaystack : () => "";
  ...
  if (empty) {
    ...
    const filtersActive = typeof deps.activityFiltersActive === "function" ? deps.activityFiltersActive(state2) : false;
    empty.textContent = hasEntries || !filtersActive ? "Nessuna attivit\xE0 registrata." : "Nessun evento corrisponde ai filtri.";
  }
  if (!hasEntries) {
    return;
  }
  filtered.forEach((entry) => {
    ...
    const safeStamp = Number.isNaN(stamp.getTime()) ? /* @__PURE__ */ new Date() : stamp;
    ...
    pinButton.textContent = entry.pinned ? "\u{1F4CC}" : "\u{1F4CD}";
...
function syncControls(state2, elements2, deps = {}) {
  state2.activityFilters = state2.activityFilters ?? {};
  if (elements2.activitySearch) {
    elements2.activitySearch.value = state2.activityFilters.query ?? "";
  }
  if (elements2.activityPinnedOnly) {
    elements2.activityPinnedOnly.checked = Boolean(state2.activityFilters.pinnedOnly);
  }
  if (Array.isArray(elements2.activityToneToggles)) {
    const defaultTones = Array.isArray(deps.defaultTones) ? deps.defaultTones : [];
    elements2.activityToneToggles.forEach((toggle) => {
      ...
      if (!tone) return;
      if (!(state2.activityFilters.tones instanceof Set)) {
        state2.activityFilters.tones = new Set(defaultTones);
      }
      toggle.checked = state2.activityFilters.tones.has(tone);
    });
  }
  if (elements2.activityTagFilter) {
    const selected = state2.activityFilters.tags ?? /* @__PURE__ */ new Set();
    Array.from(elements2.activityTagFilter.options).forEach((option) => {
      option.selected = selected instanceof Set ? selected.has(option.value) : false;
    });
  }
}

// docs/evo-tactics-pack/views/export.js
function render8(state2, elements2, deps = {}) {
  const container = elements2.exportPreview;
  const empty = elements2.exportPreviewEmpty;
  ...
  if (!hasPayload) {
    if (elements2.exportPreviewJson) elements2.exportPreviewJson.textContent = "";
    if (elements2.exportPreviewYaml) elements2.exportPreviewYaml.textContent = "";
    if (elements2.exportPreviewJsonDetails) elements2.exportPreviewJsonDetails.open = false;
    if (elements2.exportPreviewYamlDetails) elements2.exportPreviewYamlDetails.open = false;
    return;
  }
  const jsonDetails = elements2.exportPreviewJsonDetails;
  const yamlDetails = elements2.exportPreviewYamlDetails;
  const jsonWasOpen = Boolean(jsonDetails?.open);
  const yamlWasOpen = Boolean(yamlDetails?.open);
  if (elements2.exportPreviewJson) {
    elements2.exportPreviewJson.textContent = JSON.stringify(payload, null, 2);
  }
  if (elements2.exportPreviewYaml) {
    if (typeof deps.toYAML === "function") {
      elements2.exportPreviewYaml.textContent = deps.toYAML(payload);
    } else {
      elements2.exportPreviewYaml.textContent = "";
    }
  }
...
// docs/evo-tactics-pack/generator.js
var elements = resolveGeneratorElements(document);
var anchorUi = resolveAnchorUi(document);
var anchorNavigator = createAnchorNavigator();
var PACK_ROOT_CANDIDATES2 = getPackRootCandidates();
var EXPORT_BASE_FOLDER = `${PACK_PATH}out/generator/`;
var DEFAULT_ACTIVITY_TONES = ["info", "success", "warn", "error"];
var MAX_ACTIVITY_EVENTS = 150;
var DOSSIER_TEMPLATE_PATH = "../templates/dossier.html";
var initialPdfSupport = typeof window !== "undefined" && typeof window.html2pdf !== "undefined";
var NEBULA_SNAPSHOT_PATH = "../mission-console/data/nebula/atlas.json";
var TONE_LABELS = {
  info: "Info",
  success: "Successo",
  warn: "Avviso",
...
var REROLL_ACTIONS = /* @__PURE__ */ new Set(["reroll-biomi", "reroll-species", "reroll-seeds"]);
var ROLL_ACTIONS = /* @__PURE__ */ new Set(["roll-ecos", "reroll-biomi", "reroll-species", "reroll-seeds"]);
var SPECIES_ROLE_LABELS = {
  apex: "Apex",
...
var FLOW_STATUS_LABELS = {
  pending: "In attesa",
  active: "In corso",
  complete: "Completato"
};
var FLOW_STATUS_ICONS = {
  pending: "\u25CB",
  active: "\u25D4",
  complete: "\u25CF"
};
var USER_FLOWS = [
  {
    ...
    description: "Configura vincoli, filtri e profili per ottenere un roll coerente prima della generazione."
  },
  {
    id: "summary",
    label: "Riepilogo e narrativa",
    target: "#generator-summary",
    description: "Monitora metriche, narrativa dinamica e pinnature per ottimizzare la sessione in corso."
  },
  {
    id: "history",
    label: "Cronologia snapshot",
    target: "#generator-history",
    description: "Raccogli snapshot e timeline per confrontare le estrazioni e condividere i risultati."
  }
];
var PROFILE_SLOT_COUNT = 5;
var MAX_SPECIES_PER_BIOME = 3;
var MAX_HISTORY_ENTRIES = 12;
...
var MANIFEST_PRESETS = [
  {
    ...
    description: "Pacchetto completo per sessioni di prova con dati serializzati e registro attivit\xE0.",
    zipSuffix: "playtest",
    files: [
      {
        id: "ecosystem-json",
        ...
        description: (context) => `Dump completo dell'ecosistema "${context.ecosystemLabel}" con ${context.metrics.biomeCount} biomi, ${context.metrics.speciesCount} specie e ${context.metrics.seedCount} seed generati.`,
      ...
      },
      {
        ...
        description: () => "Manifesto YAML utilizzabile per commit rapidi o pipeline di integrazione continua.",
      ...
      },
      {
        ...
        description: () => "Registro attivit\xE0 in formato JSON con tutti gli eventi ordinati cronologicamente.",
      ...
      },
      {
        ...
        description: () => "Registro attivit\xE0 pronto per spreadsheet, pivot e annotazioni durante il playtest.",
  ...
  },
  {
    id: "report-design",
    label: "Report design",
    description: "Materiale di documentazione per pitch, hand-off designer e revisioni art direction.",
    zipSuffix: "report",
    files: [
      {
        id: "ecosystem-yaml",
        format: "YAML",
        filename: (slug) => `${slug}.yaml`,
        description: () => "Manifesto YAML curato per alimentare design docs, report e versioning narrativo.",
        builder: "ecosystem-yaml"
      },
      {
        id: "dossier-html",
        ...
        description: () => "Dossier HTML con panoramica visiva dei biomi, specie chiave e seed generati.",
      ...
      },
      {
        ...
        description: () => "Versione PDF del dossier, pronta per la condivisione con stakeholder esterni.",
        builder: "dossier-pdf"
      }
    ]
  },
  {
    ...
    description: "Kit leggero per distribuzioni pubbliche con manifesto, dossier e press kit narrativo.",
    zipSuffix: "demo",
    files: [
      {
        id: "ecosystem-yaml",
        format: "YAML",
        filename: (slug) => `${slug}.yaml`,
        description: () => "Manifesto YAML allineato al bundle demo, pronto per deploy statici o CDN.",
        builder: "ecosystem-yaml"
      },
      {
        id: "dossier-html",
        format: "HTML",
        filename: (slug) => `${slug}-dossier.html`,
        description: () => "Dossier HTML riassuntivo con biomi, specie in evidenza e seed narrativi per la demo.",
        builder: "dossier-html"
      },
      {
        id: "press-kit-md",
        ...
        description: () => "Press kit markdown con punti chiave, highlight e call-to-action per il lancio pubblico.",
        builder: "press-kit-md"
      }
    ]
  }
];
var DEFAULT_BRIEFING_TEXT = "Genera un ecosistema per ottenere il briefing operativo.";
var DEFAULT_HOOK_TEXT = "Gli hook narrativi compariranno dopo la prima generazione.";
...
var summaryFocusTimer = null;
var packContext = null;
var resolvedCatalogUrl = null;
var resolvedPackRoot = null;
var packDocsBase = null;
var comparisonChart = null;
var chartUnavailableNotified = false;
var composerRadarChart = null;
var composerChartUnavailable = false;
...
var parametersPanel = registerPanel("parameters", parameters_exports, () => ({
...
var traitsPanel = registerPanel("traits", traits_exports, () => ({
  renderTraitExpansions
}));
var biomesPanel = registerPanel("biomes", biomes_exports, () => ({
  renderBiomes,
  currentFilters,
  attachComparisonHandlers
}));
var seedsPanel = registerPanel("seeds", seeds_exports, () => ({
  renderSeeds
}));
var composerPanel = registerPanel("composer", composer_exports, () => ({
  renderComposerPanel,
  attachComposerHandlers
}));
var insightsPanel = registerPanel("insights", insights_exports, () => ({
...
var COMPOSER_RADAR_LABELS = ROLE_FLAGS.map((flag) => ROLE_FLAG_LABELS[flag] ?? titleCase(flag));
var TRAIT_CATEGORY_LABELS = {
...
function calculatePickMetrics() {
  ...
  const uniqueSpecies = /* @__PURE__ */ new Set();
...
function deriveFlowSnapshot() {
  ...
  const rerollCount = Number.isFinite(state.metrics?.rerollCount) ? Math.max(0, state.metrics.rerollCount) : 0;
...
function buildNarrativePrompts(pick, filters = { flags: [], roles: [], tags: [] }) {
  ...
  const speciesSummary = topSpecies.length ? topSpecies.map((entry) => {
  ...
  }).join(" | ") : "Nessuna specie prioritaria identificata";
  const highlightLabel = highlightBiome ? highlightBiome.label ?? titleCase(highlightBiome.id ?? "bioma") : "rete";
  const highlightDescriptor = highlightBiome?.synthetic ? `${highlightLabel} \xB7 Synth` : highlightLabel;
  ...
  const missionBriefingParts = [
    ...
    highlightBiome ? `Priorit\xE0 operativa: ${highlightDescriptor}.` : null,
  ...
  if (dangerousSeed) {
    ...
    const location = biomeForSeed ? biomeForSeed.label ?? titleCase(biomeForSeed.id ?? "bioma") : dangerousSeed.biome_id ?? "bioma bersaglio";
...
function buildNarrativeRecommendations({
  ...
  if (biomeId) {
    ...
    if (manifestCounts && typeof manifestCounts === "object") {
      const entries = Object.entries(manifestCounts).map(([role, value]) => ({ role, value: Number(value) })).filter((entry) => Number.isFinite(entry.value) && entry.value > 0);
    ...
    if (strongestConnection) {
      ...
      const connectedLabel = findBiomeLabelByCode(strongestConnection.to) ?? titleCase((strongestConnection.to ?? "bioma").toLowerCase());
      const resistanceLabel = typeof strongestConnection.resistance === "number" ? strongestConnection.resistance.toFixed(1) : "\u2014";
      recommendations.push({
  ...
  if (dangerousSeed) {
    ...
    const threatLabel = typeof dangerousSeed.threat_budget === "number" ? `T${dangerousSeed.threat_budget}` : null;
...
function renderContextualInsights(recommendations = []) {
  const entries = Array.isArray(recommendations) ? recommendations.filter((entry) => entry && entry.message) : [];
...
function findCatalogConnections(biomeId) {
  ...
  if (!canonical) return [];
  const connections = Array.isArray(state.data?.ecosistema?.connessioni) ? state.data.ecosistema.connessioni : [];
...
function pruneLockState() {
  ...
  const availableSpecies = /* @__PURE__ */ new Set();
...
function restoreHistoryEntries() {
  try {
    ...
    state.history = parsed.slice(0, MAX_HISTORY_ENTRIES).map((entry) => {
      if (!entry) return null;
      let timestamp = entry.timestamp ? new Date(entry.timestamp) : /* @__PURE__ */ new Date();
...
function renderHistoryPanel() {
  ...
  const appendChips = (container, values, limit) => {
    const entries2 = Array.isArray(values) ? values : [];
    if (!entries2.length) {
    ...
    entries2.slice(0, limit).forEach((label) => {
      const chip = document.createElement("span");
    ...
    if (entries2.length > limit) {
      ...
      remainder.textContent = `+${entries2.length - limit}`;
...
function persistActivityLog() {
  try {
    const serialisable = state.activityLog.map(serialiseActivityLogEntry).filter(Boolean).map((entry) => ({
      ...entry,
      timestamp: entry.timestamp
    }));
    storageService.saveActivityLog(serialisable);
  } catch (error) {
    console.warn("Impossibile salvare il registro attivit\xE0", error);
  }
}
function restoreActivityLog() {
  try {
    ...
    const restored = parsed.map((entry) => {
      ...
      const tagEntries = Array.isArray(entry.tags) ? entry.tags.map((tag) => {
        if (!tag) return null;
        if (typeof tag === "object") {
    ...
    console.warn("Impossibile ripristinare il registro attivit\xE0", error);
  }
}
...
function renderActivityTagFilters() {
  ...
  if (!select) return;
  const entries = Array.from(state.activityTagCounts.entries()).sort(
    (a, b) => a[1].label.localeCompare(b[1].label, "it", { sensitivity: "base" })
  );
  const selected = state.activityFilters.tags ?? /* @__PURE__ */ new Set();
  select.innerHTML = "";
  if (!entries.length) {
...
function buildActivityHaystack(entry) {
  ...
  return parts.filter((value) => value !== null && value !== void 0 && value !== "").join(" ").toLowerCase();
}
...
function recordActivity(message, tone = "info", timestamp = /* @__PURE__ */ new Date(), options = {}) {
  if (!message) return;
  const ts = timestamp instanceof Date ? timestamp : new Date(timestamp);
  const safeTimestamp = Number.isNaN(ts.getTime()) ? /* @__PURE__ */ new Date() : ts;
...
function recalculateActivityMetrics() {
  const entries = Array.isArray(state.activityLog) ? state.activityLog : [];
  const rollTimestamps = entries.filter((entry) => entry?.action ? ROLL_ACTIONS.has(entry.action) : false).map((entry) => {
    const stamp = entry.timestamp instanceof Date ? entry.timestamp : new Date(entry.timestamp);
...
function renderNebulaHighlights(highlights = []) {
  ...
  if (!list) return;
  list.innerHTML = "";
  const items = Array.isArray(highlights) ? highlights.filter((item) => typeof item === "string" && item.trim()).slice(0, 3) : [];
  if (!items.length) {
    return;
  }
  items.forEach((item) => {
...
function renderNebulaPanel() {
  ...
  if (!hasSnapshot) {
    if (nebulaState.error) {
      ...
      if (nebulaElements.summary) {
        nebulaElements.summary.textContent = "Impossibile raggiungere il Mission Console. Riprova pi\xF9 tardi o verifica la pubblicazione.";
      }
  ...
  if (nebulaElements.summary) {
    nebulaElements.summary.textContent = dataset?.summary || generatorMeta?.summary || "Snapshot Nebula sincronizzato dal Mission Console.";
  }
  if (nebulaElements.release) {
    nebulaElements.release.textContent = dataset?.releaseWindow ? `Finestra release: ${dataset.releaseWindow}` : "Finestra release: \u2014";
  }
  if (nebulaElements.metrics) {
    const metrics = dataset?.metrics || {};
    if (nebulaElements.metrics.species) {
      nebulaElements.metrics.species.textContent = metrics.species != null ? String(metrics.species) : "\u2014";
    }
    if (nebulaElements.metrics.biomes) {
      nebulaElements.metrics.biomes.textContent = metrics.biomes != null ? String(metrics.biomes) : "\u2014";
    }
    if (nebulaElements.metrics.encounters) {
      nebulaElements.metrics.encounters.textContent = metrics.encounters != null ? String(metrics.encounters) : "\u2014";
    }
  }
  renderNebulaHighlights(dataset?.highlights);
  const coverageAverage = telemetry?.coverage?.average;
  const coverageHistory = Array.isArray(telemetry?.coverage?.history) ? telemetry.coverage.history : [];
  ...
  if (nebulaElements.coverage) {
    const trendSuffix = typeof coverageTrend === "number" && coverageTrend !== 0 ? coverageTrend > 0 ? ` (\u2191 ${coverageTrend}%)` : ` (\u2193 ${Math.abs(coverageTrend)}%)` : "";
    nebulaElements.coverage.textContent = coverageAverage != null ? `Copertura media: ${coverageAverage}%${trendSuffix}` : "Copertura media: \u2014";
  }
  ...
  if (nebulaElements.events) {
    if (openEvents != null) {
      const highPriorityText = highPriority ? ` (${highPriority} alta priorit\xE0)` : "";
  ...
  if (nebulaElements.updated) {
    const updatedAt = nebulaState.lastSyncedAt || generatorMeta?.updatedAt || dataset?.updatedAt || summary.lastEventAt;
  ...
  const statusMessage = nebulaState.error ? `${statusLabel} \xB7 ultimo tentativo con avviso` : statusLabel;
...
async function loadNebulaSnapshot(options = {}) {
  ...
  try {
    ...
    state.nebula.lastSyncedAt = payload.updatedAt || payload.generator?.updatedAt || (/* @__PURE__ */ new Date()).toISOString();
    state.nebula.error = null;
    renderNebulaPanel();
    if (!silent) {
      const coverageAvg = state.nebula.telemetry?.coverage?.average ?? null;
      recordActivity("Snapshot Nebula sincronizzato dal Mission Console.", "info", /* @__PURE__ */ new Date(), {
        tags: ["nebula", "telemetria"],
        action: "nebula-sync-success",
        metadata: coverageAvg != null ? { coverage: coverageAvg } : void 0
      });
    }
  } catch (error) {
    ...
    renderNebulaPanel();
    if (!silent) {
      recordActivity("Impossibile sincronizzare il dataset Nebula.", "warn", /* @__PURE__ */ new Date(), {
        tags: ["nebula", "errore"],
...
function renderActivityLog() {
  render7(state, elements, getActivityViewDeps());
}
...
function resetActivityFilters() {
  ...
  state.activityFilters.tags = /* @__PURE__ */ new Set();
  state.activityFilters.tones = new Set(DEFAULT_ACTIVITY_TONES);
  syncActivityControls();
  renderActivityLog();
}
function syncActivityControls() {
  syncControls(state, elements, getActivityViewDeps());
}
function setupActivityControls() {
  setup7(state, elements, getActivityViewDeps());
}
function refreshExportSupportTelemetry() {
  ...
  if (!hasPdfSupport) {
    if (!wasNotified) {
      console.warn("html2pdf non \xE8 disponibile: esportazione PDF disabilitata.");
      recordActivity(
...
function renderExportPreview(payload) {
  render8(state, elements, { payload, toYAML });
}
...
function getPresetChecklist(presetId) {
  if (!presetId) return /* @__PURE__ */ new Map();
  if (!state.exportState.checklist.has(presetId)) {
    state.exportState.checklist.set(presetId, /* @__PURE__ */ new Map());
  }
...
function buildComposerExportSnapshot() {
  ...
  const presets = Array.isArray(state.composer?.combinedPresets) ? state.composer.combinedPresets : [];
  ...
  const heatmap = Array.isArray(metrics.heatmap) ? metrics.heatmap.map((row) => ({
    biome_id: row.biomeId,
    biome_label: row.label,
    roles: Array.isArray(row.values) ? row.values.map((cell) => ({
      flag: cell.role,
...
function buildPresetContext(filters = state.lastFilters) {
  ...
  const pinnedEntries = state.cardState?.pinned instanceof Map ? Array.from(state.cardState.pinned.values()) : [];
...
function describePresetFile(file, context) {
  ...
  const description = typeof file.description === "function" ? file.description(context) : file.description ?? "";
  let available = true;
  let reason = "";
  if (file.builder === "activity-json" || file.builder === "activity-csv") {
    if (!context.activityEntries.length) {
      available = false;
      reason = "Il registro attivit\xE0 \xE8 vuoto.";
    }
  }
  if (file.builder === "dossier-html" || file.builder === "dossier-pdf" || file.builder === "press-kit-md") {
    const hasPayload = context.metrics.biomeCount + context.metrics.speciesCount + context.metrics.seedCount > 0;
    if (!hasPayload) {
      available = false;
...
function renderExportManifest(filters = state.lastFilters) {
  ...
  const pdfNotice = pdfSupported ? "" : " \xB7 <strong>Avviso:</strong> esportazione PDF disabilitata (html2pdf non caricato).";
  meta.dataset.pdfSupported = pdfSupported ? "true" : "false";
  meta.innerHTML = `Preset <strong>${preset.label}</strong>: ${preset.description} \xB7 Cartella consigliata: <code>${context.folder}</code> \xB7 ${context.filterSummary ? `Filtri attivi: ${context.filterSummary}.` : "Nessun filtro attivo."}${pdfNotice}`;
  if (actions) {
    ...
    if (zipButton) {
      ...
      zipButton.title = !hasZipSupport ? "JSZip non disponibile. Verifica il caricamento della libreria." : "";
    }
    ...
    if (pdfButton) {
      ...
      pdfButton.title = !pdfSupported ? "html2pdf non disponibile. Controlla la connessione o ricarica la pagina." : "";
    }
  }
...
async function loadDossierTemplate2() {
  const template = await loadDossierTemplate(DOSSIER_TEMPLATE_PATH, {
    cache: {
...
async function generateDossierDocument2(context) {
  const template = await loadDossierTemplate2();
  if (!template) return null;
  return generateDossierDocument(context, {
    template,
...
async function refreshDossierPreview(context) {
  ...
  const doc = await generateDossierDocument2(context);
  if (!doc) {
    preview.innerHTML = "";
    hint.hidden = false;
...
async function generateDossierHtml2(context = buildPresetContext(state.lastFilters)) {
  return generateDossierHtml(context, {
    presetLabel: getCurrentPreset()?.label ?? null,
    roleLabels: SPECIES_ROLE_LABELS,
    titleCase,
    findBiomeLabelById,
    templateLoader: () => loadDossierTemplate2()
  });
}
async function generateDossierPdfBlob2(context) {
  const template = await loadDossierTemplate2();
  if (!template) {
    throw new Error("Template dossier non disponibile");
  }
  return generateDossierPdfBlob(context, {
    presetLabel: getCurrentPreset()?.label ?? null,
    roleLabels: SPECIES_ROLE_LABELS,
    titleCase,
    findBiomeLabelById,
    ...
    html2pdf: typeof window !== "undefined" ? window.html2pdf : void 0
  });
}
async function generatePresetFileContents2(preset, filters) {
  const context = buildPresetContext(filters);
  const { files } = await generatePresetFileContents({ files: preset.files }, context, {
...
async function downloadPresetZip(preset, filters) {
  ...
  const { files, context } = await generatePresetFileContents2(preset, filters);
  if (!files.length) {
...
function getSelectedValues(select) {
  ...
  return Array.from(select.querySelectorAll(`${MULTISELECT_OPTION_SELECTOR}[aria-pressed="true"]`)).map((button) => button.dataset.value).filter((value) => typeof value === "string" && value.length);
}
...
function parseRgbColor(value) {
  ...
  const parts = match[1].split(",").map((segment) => Number.parseFloat(segment.trim())).filter((segment) => !Number.isNaN(segment));
...
function ensurePreferredRoleSet() {
  if (!state.composer) {
    state.composer = {
      constraints: { minSynergy: 45, preferredRoles: /* @__PURE__ */ new Set() },
      combinedPresets: [],
      suggestions: [],
      metrics: { radar: [], heatmap: [] },
      roleStats: /* @__PURE__ */ new Map()
...
function syncPreferredRolesWithAvailability() {
  ...
  biomes.forEach((biome) => {
    if (!biome) return;
    collectRoles2(biome.species);
    collectRoles2(biome.generatedSpecies);
  });
  const selectedSpecies = state.pick?.species ?? {};
  Object.values(selectedSpecies).forEach((list) => collectRoles2(list));
  const removed = [];
...
function combineNames(primaryName, secondaryName) {
  const firstTokens = String(primaryName || "Alpha").split(/\s+/).filter(Boolean);
  const secondTokens = String(secondaryName || "Beta").split(/\s+/).filter(Boolean);
...
function createChipElement(value) {
  ...
  const tooltip = [];
  const glossaryEntry = state.traitGlossaryIndex instanceof Map ? state.traitGlossaryIndex.get(value) : null;
  ...
  if (info?.piSynergy?.combo_totale) {
    ...
    const formsNote = formList.length ? ` \xB7 Forme: ${formList.join(", ")}` : "";
...
var MULTISELECT_OPTION_SELECTOR = "[data-multiselect-option]";
...
function renderMultiselectPlaceholder(container, message) {
  ...
  Array.from(container.querySelectorAll(".chip-multiselect__empty")).forEach(
    (node) => node.remove()
  );
...
function ensureOption(select, value, label = value, options = {}) {
  ...
  const tooltip = options?.description ? `${label} \xB7 ${options.description}` : label === value ? label : `${label} \xB7 ${value}`;
...
function populateFilters(data) {
  ...
  if (flags.length) {
    flags.forEach(
      (flag) => ensureOption(elements.flags, flag, formatFlagLabel(flag), {
        description: SPECIES_ROLE_LABELS[flag] ?? void 0
      })
    );
  } else {
...
function indexTraitDetails(catalog) {
  ...
  Object.entries(catalog.traits).forEach(([traitId, raw]) => {
    ...
    const slotList = Array.isArray(raw?.slot ?? raw?.slots) ? (raw?.slot ?? raw?.slots).filter((item) => typeof item === "string" && item.length) : [];
    const piSynergy = raw?.sinergie_pi ?? raw?.pi_synergy ?? null;
    const environmentRequirements = Array.isArray(raw?.requisiti_ambientali) ? raw.requisiti_ambientali : Array.isArray(raw?.environment_requirements) ? raw.environment_requirements : [];
    const entry = {
      id: traitId,
...
function gatherTraitInfoForBiome(biome) {
  ...
  return {
    ...
    effects,
    description: descriptions.join(" \xB7 ") || null,
    meta: null
  };
}
function buildTraitBlock(info, { synthetic = false } = {}) {
  ...
  summary.textContent = synthetic ? `Tratti ambientali ereditati (${info.traits.length})` : `Tratti ambientali suggeriti (${info.traits.length})`;
  details.appendChild(summary);
  const chipList = document.createElement("div");
  chipList.className = "chip-list chip-list--compact";
  const sortedTraitsForChips = [...info.traits].sort(
    (a, b) => traitLabel(a).localeCompare(traitLabel(b))
  );
  ...
  orderedTraits.forEach((trait) => {
    ...
    desc.textContent = pieces.join(" \xB7 ");
...
function renderTraitExpansions() {
  ...
  orderedEntries.forEach(({ classId, entry }) => {
    ...
    meta.textContent = metaParts.join(" \xB7 ") || "Set di tratti ambientali";
    ...
    Array.from(entry.traits).sort((a, b) => traitLabel(a).localeCompare(traitLabel(b))).forEach((trait) => {
    ...
    const jobBias = Array.from(entry.jobs ?? []).filter((job) => typeof job === "string").sort((a, b) => a.localeCompare(b));
    if (jobBias.length) {
...
var CONNECTION_TYPES = ["corridor", "trophic_spillover", "seasonal_bridge"];
var SEASONALITY = ["primavera", "estate", "autunno", "inverno", "episodico", "multistagionale"];
var ENCOUNTER_BLUEPRINTS = [
  {
    id: "scout",
...
function synthesiseBiome(parents) {
  ...
  const sourceSpecies = uniqueById(
    parents.flatMap(
      (parent) => (parent.species ?? []).map((sp) => ({ ...sp, source_biome: parent.id }))
    )
  );
  ...
  const functionalGroups = /* @__PURE__ */ new Set();
...
function normaliseGeneratedSpecies(species, biomeId) {
  if (!species) return null;
  const id = species.id || `${slugify(species.display_name || species.role_trofico || "specie")}-${Math.random().toString(36).slice(2, 6)}`;
  const flags = { ...species.flags };
  ROLE_FLAGS.forEach((flag) => {
    flags[flag] = Boolean(flags?.[flag]);
  });
  const tier = Number.isFinite(species.syntheticTier) ? species.syntheticTier : inferThreatTierFromRole(species.role_trofico ?? "", flags);
  return {
    ...species,
    id,
...
function normaliseGeneratedBiome(entry) {
  ...
  const species = Array.isArray(entry.species) ? entry.species.map((sp) => normaliseGeneratedSpecies(sp, id)).filter(Boolean) : [];
...
function synthesiseConnections(biomes) {
  ...
  for (let i = 0; i < shuffled.length; i += 1) {
    ...
    connections.push({
      ...
      seasonality,
      notes: `Connessione sintetica derivata da ${current.parents?.map((p) => p.id).join("+") ?? "sorgenti ignote"} verso ${next.parents?.map((p) => p.id).join("+") ?? "target ignoto"}.`
    });
  }
  return connections;
}
...
function updateFilterSummaryHint(filters = {}) {
  ...
  if (summary) {
    ...
    elements.filtersHint.textContent = "Nessun filtro attivo. Tocca le capsule per attivare vincoli specifici.";
  }
}
...
function deriveFamiliesFromDetail(detail) {
  ...
  String(family).split(/[\\/]/g).map((part) => part.trim()).filter(Boolean).forEach((part) => {
...
function rarityFromTraitSet(traitIds) {
  const tierValues = traitIds.map((traitId) => scoreTraitTier(traitId)).filter((value) => Number.isFinite(value));
...
function threatFromTraitSet(traitIds, fallbackTier) {
  const values = traitIds.map((traitId) => scoreTraitTier(traitId)).filter((value) => Number.isFinite(value));
...
function summariseTraitDrivenProfile(traitIds) {
  const glossaryIndex = state.traitGlossaryIndex || /* @__PURE__ */ new Map();
  const detailIndex = state.traitDetailsIndex || /* @__PURE__ */ new Map();
  const labels = [];
  ...
  const summary = labels.slice(0, 3).join(" \xB7 ");
  ...
  return {
    labels,
    summary,
    description: `${descriptionParts.join(" \xB7 ")}.`,
...
function filteredPool(biome, filters) {
  ...
  return biome.species.filter(
    (sp) => matchesFlags(sp, flags) && matchesRoles(sp, roles) && matchesTags(sp, tags) && passesComposerConstraints(sp, biome)
  );
}
function generateHybridSpecies(biome, filters, desiredCount = 3) {
  ...
  while (hybrids.length < desiredCount && attempts < maxAttempts) {
    ...
    if (matchesFlags(hybrid, filters.flags) && matchesRoles(hybrid, filters.roles) && matchesTags(hybrid, filters.tags) && passesComposerConstraints(hybrid, biome)) {
      hybrids.push(hybrid);
    }
  }
  if (!hybrids.length) {
    ...
    return fallbackPool.slice(0, Math.min(desiredCount, fallbackPool.length)).map((sp) => {
      ...
      const displayName = profile.labels.length >= 2 ? `${profile.labels[0]} / ${profile.labels[1]}` : `${sp.display_name ?? sp.id} Synth`;
      const syntheticTier = Math.max(baseTier, computedTier);
      const synergyScore = profile.derived.length ? Math.min(1, profile.derived.length / Math.max(traitIds.length || 1, 1)) : 0;
      return {
        ...sp,
        ...
        statistics: {
          ...
          rarity,
          energy_profile: null,
          synergy_score: Number.isFinite(synergyScore) ? Math.round(synergyScore * 1e3) / 1e3 : 0
        },
        sources: {
          primary: {
            id: sp.id,
...
var ROLE_MATCHERS = {
  ...
  threat: (sp) => sp.flags?.threat || /predatore|incursore|menace|minaccia/i.test(sp.role_trofico ?? "") || tierOf(sp) >= 3,
  bridge: (sp) => sp.flags?.bridge || sp.flags?.event || /supporto|ponte|trasferimento/i.test(sp.role_trofico ?? "")
};
function buildCandidatePools(biome, filters) {
  ...
  const prioritized = preferGenerated ? uniqueById([...generated, ...filtered]) : uniqueById([...filtered]);
  const fallback = uniqueById([...filtered, ...nativePool]);
  const full = preferGenerated ? uniqueById([...generated, ...filtered, ...nativePool]) : uniqueById([...filtered, ...nativePool]);
...
function describeSeedNotes(biome, blueprint, filters, preferGenerated) {
  const filterSummary = summariseFilters(filters);
  if (biome.synthetic) {
    const sources = (biome.parents ?? []).map((parent) => parent.label ?? titleCase(parent.id ?? "")).filter(Boolean).join(" + ") || "fonti miste";
...
function createSeedFromBlueprint(biome, filters, blueprint) {
  ...
  return {
    ...
    synthetic: biome.synthetic || void 0,
...
function rerollSpecies(filters) {
  ...
  if (removedPreferredRoles.length && hasActiveBiomes) {
    const labels = removedPreferredRoles.map((role) => titleCase(String(role).replace(/[_-]+/g, " "))).join(", ");
    const plural = removedPreferredRoles.length > 1;
    setStatus(
      plural ? `Ruoli preferiti rimossi perch\xE9 non pi\xF9 disponibili: ${labels}.` : `Ruolo preferito rimosso perch\xE9 non pi\xF9 disponibile: ${labels}.`,
      "info",
      {
        tags: ["composer", "ruoli"],
  ...
  state.pick.biomes.forEach((biome) => {
    ...
    if (biome.synthetic) {
      ...
      const uniquePicks2 = uniqueById(picks);
      next[biomeId] = uniquePicks2.slice(0, limit);
      biome.generatedSpecies = next[biomeId];
      return;
    }
    const pool = filteredPool(biome, filters);
...
function queryPinButton(key) {
  ...
  const safeKey = typeof CSS !== "undefined" && typeof CSS.escape === "function" ? CSS.escape(key) : key.replace(/"/g, '"');
...
function initialsFromLabel(label) {
  if (!label) return "??";
  return label.split(/[^a-zA-Z0-9]+/).filter(Boolean).map((segment) => segment[0]).join("").slice(0, 2).toUpperCase();
}
...
function biomePlaceholderLabel(biome) {
  ...
  if (/foresta|bosco|giungla/.test(id)) return "\u{1F332}";
  if (/deserto|sabbia|duna/.test(id)) return "\u{1F3DC}\uFE0F";
  if (/palude|swamp|marsh|laguna/.test(id)) return "\u{1F98E}";
  if (/mont|rupe|alp/.test(id)) return "\u26F0\uFE0F";
  if (/costa|reef|mare|oceano|litor/.test(id)) return "\u{1F30A}";
  if (/vulc|lava|fuoco/.test(id)) return "\u{1F30B}";
  const initials = (biome?.label ?? biome?.id ?? "??").split(/[^a-zA-Z0-9]+/).filter(Boolean).map((segment) => segment[0]).join("").slice(0, 2).toUpperCase();
  return initials || "\u{1F728}";
}
function speciesPlaceholderIcon(species) {
  ...
  if (/predatore_terziario|apex/.test(role)) return "\u{1F988}";
  if (/predatore/.test(role)) return "\u{1F996}";
  if (/erbivoro|prede|pastoral/.test(role)) return "\u{1F98C}";
  if (/impollin/.test(role)) return "\u{1FAB2}";
  if (/detrit|saprof/.test(role)) return "\u{1FAB1}";
...
function calculateSynergy(species, biome) {
  ...
  const summary = matches.length ? `Sinergie attive: ${matches.join(", ")}` : tags.length ? `Tag funzionali da connettere: ${tags.join(", ")}` : "Nessun tag funzionale dichiarato.";
  return {
    percent,
    matches,
    tags,
...
function collectHazardIds(species) {
  const hazardIds = /* @__PURE__ */ new Set();
  const add = (value) => {
    if (!value) return;
    if (Array.isArray(value)) {
    ...
    if (typeof value === "string") {
      value.split(/[\s,;]+/).map((item) => item.trim()).filter(Boolean).forEach((item) => hazardIds.add(item));
      return;
    }
    if (typeof value === "object") {
...
function ensureComparisonChart() {
  ...
  if (typeof Chart === "undefined") {
    if (!chartUnavailableNotified) {
      console.warn("Chart.js non disponibile: il confronto radar verr\xE0 mostrato solo come elenco.");
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label(context2) {
              const detail = context2.dataset?.metaDetails ?? {};
              switch (context2.dataIndex) {
                case 0: {
                  const tier = Number.isFinite(detail.tier) ? detail.tier : context2.parsed.r;
                  return `Tier medio: ${formatTierValue(tier)}`;
                }
                case 1: {
                  const hazard = Number.isFinite(detail.hazardSignals) ? detail.hazardSignals : context2.parsed.r;
                  const label = hazard === 1 ? "segnale" : "segnali";
                  const hazardNames = Array.isArray(detail.hazardNames) ? detail.hazardNames.filter(Boolean) : [];
                  const maxDisplay = 3;
                  ...
                  return `Densit\xE0 hazard: ${hazard} ${label}${hint}`;
                }
                case 2: {
                  const roles = Number.isFinite(detail.roleTokens) ? detail.roleTokens : context2.parsed.r;
                  const label = roles === 1 ? "archetipo" : "archetipi";
                  const roleTokens = Array.isArray(detail.roleTokenList) ? detail.roleTokenList.filter(Boolean) : [];
                  const maxDisplay = 4;
                  const display = roleTokens.slice(0, maxDisplay);
                  const more = roleTokens.length > display.length ? "\u2026" : "";
                  const hint = display.length ? ` (${display.join(", ")}${more})` : "";
                  return `Diversit\xE0 ruoli: ${roles} ${label}${hint}`;
                }
                default:
                  return `${context2.label}: ${context2.formattedValue}`;
              }
            }
          }
        }
...
function renderComparisonPanel() {
  ...
  entries.forEach((entry) => {
    ...
    meta.textContent = metaParts.filter(Boolean).join(" \xB7 ");
    ...
    const hazardSignals = Number.isFinite(entry.metrics.detail.hazardSignals) ? entry.metrics.detail.hazardSignals : 0;
    const hazardLabel = hazardSignals === 1 ? "segnale" : "segnali";
    hazardMetric.innerHTML = `<span class="generator-compare__metric-label">Densit\xE0 hazard</span> ${hazardSignals} ${hazardLabel}`;
    ...
    const roleTokens = Number.isFinite(entry.metrics.detail.roleTokens) ? entry.metrics.detail.roleTokens : 0;
    const roleLabel = roleTokens === 1 ? "archetipo" : "archetipi";
    roleMetric.innerHTML = `<span class="generator-compare__metric-label">Diversit\xE0 ruoli</span> ${roleTokens} ${roleLabel}`;
...
function createSpeciesCard(biome, species) {
  ...
  const lockButton = createQuickButton({
    icon: lockActive ? "\u{1F512}" : "\u{1F513}",
  ...
  lockButton.addEventListener("click", (event) => {
    ...
    if (icon) {
      icon.textContent = locked ? "\u{1F512}" : "\u{1F513}";
    }
...
function buildBiomeCard(biome, filters) {
  ...
  const titleText = biome.synthetic ? biome.label ?? titleCase(biome.id ?? "") : titleCase(biome.id ?? "");
  const media = createCardMedia({
    label: titleText,
    variant: "biome",
  ...
  lockIcon.textContent = biomeLocked ? "\u{1F512}" : "\u{1F513}";
  ...
  lockButton.addEventListener("click", (event) => {
    ...
    lockIcon.textContent = locked ? "\u{1F512}" : "\u{1F513}";
  ...
  if (biome.synthetic) {
    const parents = (biome.parents ?? []).map((parent) => parent.label ?? titleCase(parent.id ?? "")).filter(Boolean);
    meta.textContent = parents.length ? `Origine sintetica da ${parents.join(" + ")}` : "Origine sintetica procedurale.";
  } else {
  ...
  if (biome.synthetic) {
    ...
    if (detailParts.length) {
      ...
      stats.textContent = detailParts.join(" \xB7 ");
  ...
  if (!picked.length) {
    ...
    empty.textContent = filtersText.length ? `Nessuna specie soddisfa i filtri correnti (${filtersText.join(" \xB7 ")}).` : "Nessuna specie estratta, effettua un nuovo re-roll.";
...
function buildCombinedRolePresets(roleStats) {
  ...
  const entries = Array.from(roleStats.entries()).map(([roleKey, info]) => {
    ...
    const label = roleKey ? titleCase(String(roleKey).replace(/[_-]+/g, " ")) : "Ruolo indefinito";
  ...
  const registerPreset = (roleEntries) => {
    ...
    const key = roleEntries.map((entry) => entry.role).filter(Boolean).sort().join("+");
    if (!key || seen.has(key)) return;
    const combinedFlags = /* @__PURE__ */ new Set();
    ...
    const average = totalCount ? Math.round(weightedSum / totalCount) : Math.round(roleEntries.reduce((sum, entry) => sum + entry.average, 0) / roleEntries.length);
...
function buildComposerSuggestions({ heatmap, combinedPresets, constraints, flagStats }) {
  const suggestions = [];
  const minSynergy = Number.isFinite(constraints?.minSynergy) ? constraints.minSynergy : 0;
  const populatedRows = Array.isArray(heatmap) ? heatmap.filter(
    (row) => Array.isArray(row?.values) && row.values.some((cell) => cell.count > 0)
  ) : [];
  if (populatedRows.length) {
    const lowCells = [];
    populatedRows.forEach((row) => {
      row.values.filter((cell) => cell.count > 0).forEach((cell) => {
    ...
    if (lowCells.length) {
      ...
      suggestions.push({
        tone: "warn",
        message: `${critical.row.label}: sinergia ${critical.cell.average}% su ${ROLE_FLAG_LABELS[critical.cell.role] ?? critical.cell.role}. Valuta un preset mirato o abbassa la soglia.`
      });
  ...
  if (flagStats instanceof Map) {
    ...
    if (missingRoles.length) {
      suggestions.push({
        tone: "info",
        message: `Nessuna specie con flag ${missingRoles.map((flag) => ROLE_FLAG_LABELS[flag] ?? flag).join(", ")} nei roll correnti.`
      });
    }
  }
...
function renderComposerHeatmap(rows) {
  ...
  rows.forEach((row) => {
    ...
    ROLE_FLAGS.forEach((flag) => {
      const cell = Array.isArray(row.values) ? row.values.find((entry) => entry.role === flag) : null;
      ...
      cellElement.title = count ? `${count} specie \xB7 ${ROLE_FLAG_LABELS[flag] ?? flag} \xB7 sinergia ${average}%` : `Nessuna specie con flag ${ROLE_FLAG_LABELS[flag] ?? flag}`;
...
function ensureComposerRadarChart() {
  ...
  if (typeof Chart === "undefined") {
    if (!composerChartUnavailable) {
      console.warn(
        "Chart.js non disponibile: il radar sinergie della composizione avanzata rimarr\xE0 nascosto."
      );
...
function renderComposerPanel() {
  ...
  if (presetList) {
    ...
    if (hasPresets) {
      presets.forEach((preset) => {
        ...
        const roleMetric = document.createElement("span");
        roleMetric.textContent = `Ruoli: ${preset.filters.roles.map((role) => titleCase(String(role).replace(/[_-]+/g, " "))).join(", ")}`;
        metrics.appendChild(roleMetric);
  ...
  if (toggles) {
    toggles.innerHTML = "";
    const roleStats = composerState.roleStats instanceof Map ? composerState.roleStats : /* @__PURE__ */ new Map();
    const roleEntries = Array.from(roleStats.entries()).map(([roleKey, info]) => {
      const count = info?.count ?? 0;
...
function updateComposerState(filters) {
  ...
  const roleStats = /* @__PURE__ */ new Map();
...
function applyComposerConstraintUpdate({ announce = false } = {}) {
  ...
  renderSeedsPanel();
  if (announce) {
    const value = Number.isFinite(state.composer?.constraints?.minSynergy) ? state.composer.constraints.minSynergy : 0;
...
function renderBiomes(filters) {
  ...
  if (removed) {
    ...
    setStatus(`${label} rimosse dal confronto perch\xE9 non pi\xF9 disponibili.`, "warn", {
      tags: ["confronto", "sync"],
...
function renderSeeds() {
  ...
  state.pick.seeds.forEach((seed) => {
    ...
    header.innerHTML = `${headingParts.join(" \xB7 ")} \xB7 <span class="form__hint">Budget T${seed.threat_budget}</span>`;
    ...
    if (!seed.party.length) {
      ...
      seed.party.forEach((entry) => {
        ...
        item.innerHTML = `${entry.display_name} <span class="form__hint">(${parts.join(" \xB7 ")})</span>`;
        list.appendChild(item);
      });
    }
    card.append(list);
...
function exportPayload(filters) {
  const catalogSource = resolvedCatalogUrl || packContext?.catalogUrl || resolvedPackRoot || packContext?.resolvedBase || null;
  return {
    pick: state.pick,
    filters,
    source: {
...
function exportActivityLogEntries() {
  ...
  return state.activityLog.map(serialiseActivityLogEntry).filter(Boolean).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
}
    ...
    if (result?.data) {
      ...
      if (result.fromFallback) {
        setStatus("Registro tratti caricato dal fallback locale.", "info", {
          ...
          metadata: { url: result.url ?? void 0 }
        });
      }
      return;
    }
  ...
  elements.form.addEventListener("click", async (event) => {
    ...
    switch (action) {
      case "roll-ecos": {
        ...
        const metaSuffix = summaryParts.length ? ` (${summaryParts.join(" \xB7 ")})` : "";
        setStatus(
          ...
          "success",
          {
            ...
            rareTarget: narrativeContext?.rareReason ?? void 0
          }
        );
      ...
      case "reroll-biomi": {
        ...
        const rerollSuffix = rerollSummary.length ? ` (${rerollSummary.join(" \xB7 ")})` : "";
      ...
      case "export-log-json": {
        if (!state.activityLog.length) {
          setStatus("Il registro attivit\xE0 \xE8 vuoto, nulla da esportare.", "warn", {
        ...
        renderParametersPanel({ refresh: ["export"], filters });
        setStatus("Registro attivit\xE0 esportato in JSON.", "success", {
      ...
      case "export-log-csv": {
        ...
        renderParametersPanel({ refresh: ["export"], filters });
        setStatus("Registro attivit\xE0 esportato in CSV.", "success", {
          tags: ["export", "log", "csv"],
      ...
      case "download-dossier-html": {
        try {
          const context = buildPresetContext(filters);
          const html = await generateDossierHtml2(context);
          if (!html) {
      ...
      case "download-dossier-pdf": {
        try {
          const context = buildPresetContext(filters);
          const blob = await generateDossierPdfBlob2(context);
          const slug = ensureExportSlug();
...
if (typeof window !== "undefined") {
  window.EvoPack = window.EvoPack || {};
  window.EvoPack.packRootCandidates = PACK_ROOT_CANDIDATES2;
