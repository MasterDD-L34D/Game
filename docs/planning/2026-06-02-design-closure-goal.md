...
> ⚠️ **QUESTA INVENTORY È DA MEMORY (STALE-PRONE) — VERIFY-FIRST OGNI RIGA su origin/main PRIMA di lavorarla.**
> Costruita 2026-06-02 da memory; **già 3 voci risultate GIÀ-SHIPPED** sotto verify-first (Codex #2551):
> **H1** GAP-A (#2447), **H3** campaign-XP (#2550), **H5** symbiont/minion (#2539-#2549) → marcate ✅ done.
> **Buchi genuinamente APERTI** (al 2026-06-02, comunque ri-verifica): **H2** economy cost-gate (decorativo confermato `abilityExecutor.js:2374-2379`), **H4** Cat F roll-tags residui, **H6** ecosystem-tier design-Q, **H7** pillar surface-audit, **H8** 1-HP cleanup, **H9** OD residui. Lezione: stesso pattern #19 (5× questa sessione) — `git log origin/main -S <simbolo>` PRIMA di trattare una riga come aperta.
