...
| --------------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `.github/workflows/ci.yml`                    | dev-tooling      | Log assente – dispatch manuale pianificato (2025-12-02) per riattivare run completo                  | enforcing                     | Richiesto run completo pre-go-live; verificare suite dashboard/CLI e ripristinare host prima del rerun.                          |
| `.github/workflows/e2e.yml`                   | QA               | Log assente – dispatch manuale pianificato (2025-12-02) verso nuovo host staging                     | enforcing                     | Necessario Playwright su host raggiungibile; riallineare BASE_URL prima del rerun.                                               |
| `.github/workflows/daily-pr-summary.yml`      | archivist        | Log assente – verificare cron/manuale                                                                | enforcing                     | Auto-commit, nessun blocco noto.                                                                                                 |
| `.github/workflows/daily-tracker-refresh.yml` | analytics        | Log assente – verificare cron/manuale                                                                | enforcing                     | Auto-commit; verificare credenziali scheduler.                                                                                   |
...
| `.github/workflows/deploy-test-interface.yml` | devops           | Log assente – dispatch manuale pianificato (2025-12-02) per dry-run Pages                            | enforcing                     | Dry-run richiesto con bundle HUD/CLI; attendere host stabile prima di rilanciare.                                                |
| `.github/workflows/idea-intake-index.yml`     | archivist        | Log assente – verificare ultimo auto-commit                                                          | enforcing                     | Auto-commit; controllare diff indice.                                                                                            |
| `.github/workflows/incoming-smoke.yml`        | dev-tooling      | 2026-07-24 – PASS (redirect smoke) 【F:reports/redirects/redirect-smoke-2026-07-24.json†L1-L11】     | enforcing                     | Host `http://localhost:8000` raggiungibile; mantenere mapping R-01/R-02/R-03 e rieseguire dopo eventuali rebase su incoming.     |
| `.github/workflows/validate-naming.yml`       | data             | 2025-11-30 – KO (glossario incompleto) 【F:logs/ci_runs/validate-naming_run3.log†L1-L8】             | enforcing                     | Glossario aggiornato con `ali_solari_fotoni`; pianificare rerun per chiudere il KO naming.                                       |
...
| `.github/workflows/qa-kpi-monitor.yml`        | QA               | Log assente – raccogliere run mensile/manuale                                                        | enforcing                     | Nessun log locale; controllare `logs/visual_runs` nel prossimo ciclo.                                                            |
| `.github/workflows/qa-export.yml`             | QA               | Log assente – dispatch manuale pianificato (2025-12-02)                                              | enforcing                     | Export QA manuale da rilanciare una volta ripristinato l'endpoint di staging per allegare artefatti aggiornati.                  |
| `.github/workflows/qa-reports.yml`            | QA               | Log assente – dispatch manuale pianificato (2025-12-02)                                              | enforcing                     | Run manuale richiesto per aggiornare `reports/qa_badges.json` e baseline appena l'host torna raggiungibile.                      |
| `.github/workflows/telemetry-export.yml`      | analytics        | Log assente – validare export recente                                                                | enforcing                     | Validare export/schema prima del go-live.                                                                                        |
| `.github/workflows/search-index.yml`          | web perf/content | Log assente – verificare ultimo aggiornamento indice                                                 | enforcing                     | Verificare generazione `public/search_index.json`.                                                                               |
| `.github/workflows/hud.yml`                   | HUD              | Log assente – dispatch manuale pianificato (2025-12-02)                                              | enforcing (salta se flag off) | Necessario abilitare flag HUD e ottenere build verde; rerun dopo ripristino host per allegare artefatti overlay.                 |
| `.github/workflows/lighthouse.yml`            | web perf         | Log assente – attendere/forzare run                                                                  | report-only                   | Attendere run schedulato/manuale per score aggiornati.                                                                           |
| `.github/workflows/update-evo-tracker.yml`    | roadmap          | Log assente – validare job richiamato                                                                | enforcing                     | Nessun blocco noto; input `batch` opzionale.                                                                                     |
| `.github/workflows/traits-sync.yml`           | data             | Log assente – schedulato/manuale                                                                     | enforcing                     | Verificare segreti S3 se pubblicazione attiva.                                                                                   |
| `.github/workflows/evo-batch.yml`             | ops              | Log assente – dry-run/manuale                                                                        | enforcing (dry-run default)   | Run manuale con `execute=true` solo dopo revisione batch.                                                                        |
| `.github/workflows/evo-doc-backfill.yml`      | archivist        | Log assente – schedulato/manuale                                                                     | enforcing                     | Controllare diff/backfill prima di notifiche.                                                                                    |
| `.github/workflows/evo-rollout-status.yml`    | roadmap          | Log assente – schedulato/manuale                                                                     | enforcing                     | Verificare snapshot `reports/evo/rollout/status_export.json`.                                                                    |
