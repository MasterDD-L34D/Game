...
- Il design di PATCHSET-00 è completato (versione 0.2 → 0.5) e resta in attesa di approvazione per eseguire la sequenza 01A–03B.
...
### Gestione conflitti PR

- Prima di aprire/aggiornare PR, riallineare il branch su `main` (merge o rebase) e annotare l’operazione in `logs/agent_activity.md`.
- In caso di conflitto, risolvere localmente e aggiornare la sezione `Changelog` del documento toccato indicando l’esito (es. "conflitto risolto contro main @<commit>").
- Se il conflitto riguarda dati/tooling, fermarsi e aprire un gate umano aggiuntivo prima di procedere con 02B/03B.
...
- Popolare `REF_INCOMING_CATALOG` con tabella di stato per `incoming/` e `docs/incoming/` e collegare alle fasi 01B/02A (pronto per 01A in v0.5).
