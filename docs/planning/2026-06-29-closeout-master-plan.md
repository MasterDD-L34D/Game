...
| I3     | **SPEC-F durable crossbreed cooldown**               | **DEFERRED 2026-06-30** (PR #3101 PREPARED then CLOSED). Verify-first: premise undermined -- `hydrateAsync` is NEVER called, so on a fresh process `getCompanionState` 404s before the cooldown check -> restart resets companion-state + cooldown TOGETHER = no reachable bypass + zero real durability from a schema field (+ FIFO-eviction edge). Durable cooldown = persistence-layer workstream (hydrate the store / direct-Prisma key), SAME gap class as I4. Blocker now OWNER(defer) | OWNER-merge   | S        |
