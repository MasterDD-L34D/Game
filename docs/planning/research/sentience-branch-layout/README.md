...
**Purpose.** Questa branch introduce il **MVP per i Tratti di Sensienza (T1–T6)** e l’ossatura dati ispirata ad _Ancestors: The Humankind Odyssey_, per migliorare specie e comportamenti nel progetto **Evo Tactics**.
...
- `CHANGELOG.md` — Changelog in formato _Keep a Changelog_.
...
> **Note GitHub:** il **pull request template** deve esistere nella _default branch_ (o nella directory `.github/` della repo) per essere proposto automaticamente quando apri la PR. In alternativa, mantienilo qui e copialo poi in `main`/`default`.
...
- **Changelog:** formato _Keep a Changelog_ con SemVer.
