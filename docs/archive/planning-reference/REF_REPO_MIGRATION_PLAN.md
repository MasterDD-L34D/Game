---
title: REF_REPO_MIGRATION_PLAN – Sequenza patchset
doc_status: draft
...
**Nota allineamento smoke redirect 2026-09-08T1200Z:** ultimo smoke **PASS** su host `http://localhost:8000` con log `[REDIR-SMOKE-2026-09-08T1200Z]` in [logs/agent_activity.md](../../logs/agent_activity.md) e report [reports/redirects/redirect-smoke-staging.json](../../reports/redirects/redirect-smoke-staging.json); ticket #1204/#1205 confermati **Approved**, #1206 in **Ready/Approved** come baseline di rollback (host ripristinato dopo l'errore CONNECTION REFUSED dello stesso giorno).
...
- `[REDIR-SMOKE-2026-09-08T1200Z]` – smoke redirect **PASS** su host `http://localhost:8000` (`reports/redirects/redirect-smoke-staging.json`), con precedente esito **ERROR** (Connection refused) nello stesso slot risolto al rerun; ticket #1204/#1205 in Approved e #1206 consolidato Ready/Approved, mapping TKT-03B-001 riallineato alla milestone 07/12/2025.
- **Aggiornamento redirect (staging)** – l’ultimo ciclo di smoke 2026-09-08 contiene sia il fallimento iniziale (host non raggiungibile) sia il rerun **PASS** loggati in [logs/agent_activity.md](../../logs/agent_activity.md); mantenere entrambi gli esiti come storico e puntare alla versione PASS come baseline. Prima di eventuali nuovi rerun, riconfermare i manifest di backup `reports/backups/2025-12-01_redirect_staging/redirect-config/manifest.txt` e `reports/backups/2025-11-25_freeze/manifest.txt`.
- `[03A03B-ADVANCE-2026-09-08T1230Z]` – avanzamento 03A/03B confermato con freeze attivo: TKT-03A-001 e TKT-03B-001 marcati Ready con allegati di riferimento (02A remediation + smoke redirect 2026-09-08 **PASS**); milestone operativa confermata al 07/12/2025 con finestra alternativa 2025-12-09 per rollback.
...
- **Branch attivi e baseline di sblocco 03A/03B:** `patch/03A-core-derived` (rollout patch core/derived) e `patch/03B-incoming-cleanup` (pulizia/archivio incoming). I ticket **[TKT-03A-001]** e **[TKT-03B-001]** restano **Ready** solo se due evidenze restano **PASS** e attuali: (1) validator 02A in modalità report-only `[02A-REMEDIATION-2025-12-08T1030Z]` con report [reports/02A_validator_rerun.md](../../reports/02A_validator_rerun.md) e bundle `reports/temp/02A_rerun_20251201/`; (2) smoke redirect `[REDIR-SMOKE-2026-09-08T1200Z]` (rerun **PASS** con storico dell’ERROR Connection refused nello stesso slot) con output [reports/redirects/redirect-smoke-staging.json](../../reports/redirects/redirect-smoke-staging.json). Lo stato Ready/Approved dei ticket #1204/#1205/#1206 è ancorato a questi log in [logs/agent_activity.md](../../logs/agent_activity.md); qualsiasi rerun successivo deve essere loggato nello stesso file per mantenere la baseline.
- **Finestra di freeze vs fallback:** il freeze 03A/03B resta attivo su `core/**`, `derived/**`, `incoming/**` finché entrambi i report sopra restano la baseline valida e si completa la firma Master DD; la finestra alternativa di rollback rimane 2025-12-09T09:00Z → 2025-12-09T18:00Z (staging e redirect), separata dalla finestra principale 2025-11-29 → 2025-12-07.
- **Criteri di stop e riesecuzione:** bloccare merge se il validator 02A report-only segnala regressioni o se il smoke redirect riporta nuovi ERROR/FAIL; in tal caso rieseguire smoke su `http://localhost:8000` aggiornando [reports/redirects/redirect-smoke-staging.json](../../reports/redirects/redirect-smoke-staging.json) e log correlato in [logs/agent_activity.md](../../logs/agent_activity.md) prima di riaprire i gate 03A/03B.
- **Criteri di rollback:** ripristinare snapshot core/derived e backup incoming etichettati pre-merge (manifest in `reports/backups/**`), riattivare redirect originali e rieseguire validator 02A + smoke redirect per confermare il ritorno allo stato stabile. La finestra alternativa 2025-12-09T09:00Z → 2025-12-09T18:00Z resta dedicata ai test di rollback e alle convalide post-restore.
...
| Readiness 01B/01C ([nota readiness](#nota-readiness-01b01c-agg-2026-09-11)) | species-curator (lead 01B), trait-curator, balancer, archivist, coordinator | Dopo gap list approvata | On-call registrata il 2025-02-07; kickoff 01B rieseguito e loggato 2025-07-09 su `patch/01B-core-derived-matrix` (perimetro core/derived + ticket **[TKT-01A-001..005]/[TKT-01B-001]/[TKT-01B-002]**); readiness aggiornata 2025-07-10 con freeze incoming attivo 2025-11-29→2025-12-07 (nuova finestra allineata alla milestone 2025-12-07) e owner 01C su `patch/01C-tooling-ci-catalog` marcato report-only. Chiusura/fix documentale registrata in `[01B-CLOSE-FIX-2026-09-11T1200Z]` e `[01C-CLOSE-FIX-2026-09-11T1200Z]` su [logs/agent_activity.md](../../logs/agent_activity.md) con conferma freeze chiuso. |
| Inventario tooling/CI 01C ([REF_TOOLING_AND_CI](./REF_TOOLING_AND_CI.md#inventario-workflowscript-modalita-report-only--2026-02-07)) | dev-tooling (lead 01C) + coordinator | Successivo a via libera 01A/01B | Attivato (log 2025-04-12) e confermato report-only 2025-07-09 su `patch/01C-tooling-ci-catalog` con trigger di avanzamento/rollback documentati; pronta l’esecuzione report/enforcing secondo gating 02A; consolidamento in `[01C-CLOSE-FIX-2026-09-11T1200Z]` (report-only) in [logs/agent_activity.md](../../logs/agent_activity.md). |
...
### Nota readiness 01B/01C (agg. 2026-09-11)

- **On-call 01B (core/derived):** species-curator (lead), trait-curator (nomenclature/trait mapping), balancer (priorità P0/P1/P2), archivist (cross-check catalogo 01A), coordinator (gate/log). Input di kickoff: gap list 01A approvata con ticket **[TKT-01A-LDC]**, **[TKT-01A-ANC]**, **[TKT-01A-PARAM]**, **[TKT-01A-ENGINE]**, **[TKT-01A-DOCS]** registrati nei README incoming; branch di lavoro `patch/01B-core-derived-matrix` confermato con rerun 2025-07-09; chiusura/fix report-only a log `[01B-CLOSE-FIX-2026-09-11T1200Z]` su [logs/agent_activity.md](../../logs/agent_activity.md).
- **On-call 01C (tooling/CI):** dev-tooling (lead) con coordinator per priorità; inventario e controlli in modalità **report-only** su `patch/01C-tooling-ci-catalog`, nessun rollout esecutivo finché 01A–01B non sono loggati con approvazione Master DD; consolidamento in `[01C-CLOSE-FIX-2026-09-11T1200Z]` su [logs/agent_activity.md](../../logs/agent_activity.md).
- **Freeze e ticket attivi:** finestra documentale 2025-11-29T09:00Z → 2025-12-07T18:00Z (allineata alla milestone 2025-12-07, delta -1 anno vs vecchia 2026-07-08→2026-07-15) su `incoming/**` e `docs/incoming/**` (parcheggio in `_holding`); ticket attivi **[TKT-01B-001]**, **[TKT-01B-002]**, **[TKT-01C-001]**, **[TKT-01C-002]** con logging continuo in STRICT MODE; freeze confermato chiuso nei log di chiusura fix 2026-09-11.
...
- **Rischi aperti:** freeze non riattivato (nuova approvazione richiesta), possibile desincronizzazione tra catalogo e README se si aggiungono batch senza loggare; mantenere allineati i log 02A/03A/03B per evitare drift tra runbook e branch; attendere firma finale Master DD sui log `[01B-CLOSE-FIX-2026-09-11T1200Z]`/`[01C-CLOSE-FIX-2026-09-11T1200Z]` prima di qualsiasi riapertura.
