...
1. **Riorganizzare i job per fasi logiche**  
   ...
   - _Origine_: mantenere il job `paths-filter` per determinare l’area impattata e generare output booleani.
   - _Build_: un job unico per compilare i componenti TS (CLI e webapp) e Python se necessario, usando `npm run build` e `python setup.py`/`pip install -e` con caching centralizzato.
   - _Test_: raggruppare i test unitari TS e Python e la validazione dei dataset in due job distinti (frontend/backend/dataset). Usare matrice di esecuzione per versioni Node/Python diverse solo se necessario.
   - _Deploy_: mantenere il job `deployment-checks` con i passaggi di validazione e smoke test, ma prevedere step condizionati per release taggate.
