...
| `VC_AXES_ITER`                 | unset → auto (iter2 ≥10 evt, iter1 <10)            | `apps/backend/services/vcScoring.js:895-909`         | Force MBTI axes iteration version. `1` = iter1 legacy (back-compat), `2` = iter2 4-axis full reveal. Default UNSET: runtime auto-picks iter2 quando `events_count >= 10` (production-typical), iter1 fallback per tutorial seed/short sessions. Set `VC_AXES_ITER=2` per playtest reveal A/B forzato. Codex PR #2348 audit fix: doc precedente "1 (auto)" sbagliato.                                        |
| `MBTI_REVEAL_THRESHOLD`        | unset → 0.7 (or 0.6 for short sessions <30 events) | `apps/backend/services/mbtiSurface.js:143-170`       | Phased reveal MBTI confidence threshold (Disco Elysium pattern). Range `[0..1]`. Default UNSET: runtime baseline `0.7`, BUT abbassa auto a `0.6` quando `events_count < 30` (short session boost). Set `MBTI_REVEAL_THRESHOLD=0.5` per relaxed reveal A/B test forzato (più assi early game) — env override disabilita auto-boost dinamico. Codex PR #2348 audit fix: doc precedente "0.7" fisso sbagliato. |
