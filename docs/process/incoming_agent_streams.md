...
- Script `scripts/report_incoming.sh` e log generati in `reports/incoming/` per alimentare le analisi successive.【F:docs/process/incoming_triage_pipeline.md†L18-L44】【F:docs/incoming/README.md†L5-L28】
...
### Responsabilità principali
- Eseguire e monitorare i comandi di validazione (`tools/py/game_cli.py`, `tools/ts/validate_species.ts`) generati dal triage, segnalando regressioni critiche e completando la colonna `In validazione` con log allegati.【F:docs/process/incoming_triage_pipeline.md†L12-L57】【F:docs/incoming/README.md†L16-L28】
...
### Output attesi
- Log di validazione consolidati in `reports/incoming/validation/` e allegati alle card Kanban per asset in analisi.【F:docs/process/incoming_triage_pipeline.md†L46-L56】【F:docs/incoming/README.md†L16-L24】
...
### Dipendenze chiave
- Script CLI Python/TypeScript e dataset `incoming/` estratti dal triage per riprodurre i test.【F:docs/process/incoming_triage_pipeline.md†L12-L57】【F:docs/incoming/README.md†L5-L28】
...
- Evidenziare l'impatto dei biomi sulle specie, morph e encounter condivisi nel Feature Map EVO Tactics.【F:docs/incoming/FEATURE_MAP_EVO_TACTICS.md†L55-L76】【F:docs/incoming/FEATURE_MAP_EVO_TACTICS.md†L86-L94】
...
- Raccolta di insight su hazard, preferenze e hook ambientali da condividere con gli stream Core/Validation.【F:docs/incoming/FEATURE_MAP_EVO_TACTICS.md†L55-L76】
...
### Responsabilità principali
- Gestire i moduli Enneagramma/MBTI, assicurando che dataset, compat map e registry siano sincronizzati con le istruzioni di integrazione meccanica.【F:docs/process/incoming_triage_pipeline.md†L15-L92】【F:docs/incoming/README_INTEGRAZIONE_MECCANICHE.md†L5-L14】
- Validare gli hook e le mappe stat/eventi contro il repository principale seguendo il flusso di scanning e binding fornito negli asset incoming.【F:docs/incoming/GAME_COMPAT_README.md†L6-L18】【F:docs/incoming/README_SCAN_STAT_EVENTI.md†L4-L17】
- Fornire agli altri stream esempi di binding e limiti di stacking per evitare regressioni durante l'integrazione.【F:docs/incoming/GAME_COMPAT_README.md†L14-L18】【F:docs/incoming/hook_bindings.ts†L1-L38】

### Output attesi
- Checklist di compatibilità aggiornata con hook abilitati e test smoke sui 108 profili indicati.【F:docs/incoming/GAME_COMPAT_README.md†L14-L18】
- Aggiornamenti a `compat_map.json`/`personality_module.v1.json` con alias e limiti confermati in fase di triage.【F:docs/incoming/README_INTEGRAZIONE_MECCANICHE.md†L5-L14】
...
### Dipendenze chiave
- Pacchetto Personality (`compat_map.json`, `personality_module.v1.json`, `hook_bindings.ts`) e dataset temi per garantire coerenza tra script e engine.【F:docs/incoming/FEATURE_MAP_EVO_TACTICS.md†L32-L43】【F:docs/incoming/hook_bindings.ts†L1-L38】
- Scanner `scan_engine_idents.py` e workflow di validazione repo per allineare naming STAT/EVENTI.【F:docs/incoming/README_SCAN_STAT_EVENTI.md†L4-L17】
- Supporto di `AG-Core` per armonizzare i limiti con le statistiche canoniche (baseline conservativa).【F:docs/incoming/README_INTEGRAZIONE_MECCANICHE.md†L3-L14】【F:docs/process/incoming_triage_pipeline.md†L76-L82】
...
- Script e configurazioni aggiornati per assicurare estrazione non interattiva, validazioni schema e sincronizzazione addon.【F:docs/process/incoming_triage_pipeline.md†L18-L124】【F:docs/incoming/README.md†L5-L28】
