...
| `data/traits/index.json`                                                                         | Sorgente autorevole per tier, slot, sinergie, requisiti ambientali e metadati PI.                                                                                               | Proiettato (mirror lean) in `packs/evo_tactics_pack/docs/catalog/trait_reference.json` (bundle pack) e da li' copiato in `docs/`/`public/`; i mirror sono **generati** da `index.json` via `npm run sync:evo-pack` -- non modificarli a mano. |
...
2. **Aggiornare il trait reference** – editare `data/traits/index.json` (sorgente unica), poi rigenerare i mirror con `npm run sync:evo-pack`. Lo step proietta `index.json` in `packs/evo_tactics_pack/docs/catalog/trait_reference.json` (via `scripts/sync_trait_reference.js`) e propaga a `docs/` + `public/`. NON editare i mirror a mano: il guard CI "Verify evo-pack catalog is regenerated (no drift)" fallisce se divergono da `index.json`.
