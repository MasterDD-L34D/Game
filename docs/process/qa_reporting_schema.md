...
- **Validazione schema automatica** — `python tools/py/validate_export.py --export data/exports/qa-telemetry-export.json --recipients config/drive/recipients.yaml` controlla che ogni record rispetti i campi obbligatori, enum (`priority`, `status`), pattern milestone e che i gruppi destinatari siano definiti nel file di configurazione con finestre valide.【F:tools/py/validate_export.py†L1-L211】【F:config/drive/recipients.yaml†L1-L57】【F:data/exports/qa-telemetry-export.json†L1-L38】
