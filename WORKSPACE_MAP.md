...
- [ ] **HTTP Alt B runtime smoke (Game backend live)** — ready, richiede `npm install` nel worktree (~5min) per risolvere `@game/contracts`. Comando: `cd Game/ && npm install && GAME_DATABASE_ENABLED=true GAME_DATABASE_URL=http://localhost:3333 LOBBY_WS_ENABLED=false node apps/backend/index.js` poi `curl http://localhost:3334/api/catalog/pools` con Game-Database :3333 up. Userland o post-merge PR #1804 quando worktree merge in main.
