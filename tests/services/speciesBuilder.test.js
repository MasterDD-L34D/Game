...
test('createSpeciesBuilder exposes metadata for traits', async () => {
  ...
  assert.ok(
    metadata.usage_tags.includes('scout'),
    'aggregated usage tags should include trait tags',
  );
  ...
  assert.equal(
    metadata.completion_flags.has_species_link,
    true,
    'aggregated completion flags should be truthy for species link',
  );
});
