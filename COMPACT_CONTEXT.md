...
**(A) Tier-2 design-call batch DRENATO**: **O3** ratify PILLAR 6/6 + restore P1 def++ #3096 `a6e0c034` (additive delta, tabella + P5 caveat intatti, registry atomico) · **O5** D7 branco-tendency prose form-B per-bioma #3097 `ffe4be8a` (backend flat->per-bioma key + 7 it/en; surface Godot-only=copy-readiness, flag OFF) · **O1** SCAR_TRAIT_MAP ratify + `testa->occhi_cristallo_modulare` #3098 `2b9e978c` (LIVE, 33/33 nido, flag OFF) · **O7** GAP2-next confermato DEFERRED + fix conteggio.

**(B) Infra I3/I4 ENTRAMBI DEFERRED**: **I3** crossbreed cooldown durable = PR #3101 PREPARED poi **CHIUSO** (verify-first: `hydrateAsync` mai chiamato -> companion store non idrata -> fresh process 404 prima del cooldown -> zero durabilita' reale da campo schema; il test passava solo perche' stub in-memory condivide stato). **I4** ritual resource-cost E6 = BLOCKED (no resource pool backend). Root-cause comune = persistence-layer SPEC-F/E non wired cross-restart -> `TKT-PERSISTENCE-LAYER`.

**Codex async-cascade**: 6 doc-fix PR (#3099->#3105) -- un fix consistenza-marker triggera il review successivo che trova il prossimo entry-point stale (plan/register/2 handoff/BACKLOG/colonne-blocker); reply+resolve ogni thread, 0 unresolved. 2 cavecrew (isolation worktree) sui code-PR.

**Lessons**: (1) test durabilita' cross-restart DEVE verificare che lo store reale IDRATI (stub in-memory condivide stato = maschera no-hydration gap); PR forbidden-path con premessa non-verificata = peggio di nessuna. (2) Codex async-cascade: grep TUTTI gli entry-point quando chiudi una riga. (3) recon-first fan-out prima di ogni batch owner-decision = ground-truth (catch O7-already-deferred, O5 slug-vs-test, I4 no-pool, I3 no-hydration).

**Next entry point** (owner-gated, archi dedicati): **Tier-3 N=40** (N1 SPEC-J LETHAL primo: autora encounter lethal:true -> N=40 G2 -> flip permadeath) OPPURE **`TKT-PERSISTENCE-LAYER`** (sblocca I3+I4) · Godot G2/G3/G6 · B6 keeper content-debt. ⚠️ re-sweep Codex su #3097/#3098 (code-PR, cavecrew-clean, non ancora visti). Presenta la prossima lane via AskUserQuestion.
