...
def check_playable_and_budget(sp, catalog_slots, gr, cfg):
    ...
    if not sp.get('playable_unit'):
        if role in {'produttore','produttore_chemiotrofo','decompositore','ingegneri_ecosistema'}:
...
def check_jobs_mapping(sp, cfg):
    ...
    return True
def check_mate_synergy(sp, cfg):
    ms = sp.get('mate_synergy') or []
    if not ms: return True
    allowed = set(cfg.get('mate_archetypes',[]) or [])
    for a in ms:
        if a not in allowed:
            sev(f"{sp['id']}: mate_synergy '{a}' fuori enum mate_archetypes", 'WARNING')
    if not sp.get('flags',{}).get('sentient') and not (sp.get('mating') or {}).get('enabled'):
        sev(f"{sp['id']}: mate_synergy presente ma specie non sentiente e mating.disabled", 'WARNING')
    return True
