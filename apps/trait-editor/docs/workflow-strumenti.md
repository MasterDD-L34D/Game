...
3. **Verificare l'anteprima nel Trait Editor standalone**
   - Avvia `npm run dev` da `apps/trait-editor/`.
...
| Anteprima editor standalone | `npm run dev` (da `apps/trait-editor/`, con `VITE_TRAIT_DATA_SOURCE`, `VITE_TRAIT_DATA_URL`)                                              | Interfaccia AngularJS con sync remoto e fallback mock automatico.                      |
