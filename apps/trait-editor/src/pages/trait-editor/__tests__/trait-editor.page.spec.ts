...
const createController = (
  trait: Trait,
  validationResult: TraitValidationResult,
): { controller: TraitEditorController; deps: ControllerDeps } => {
  const scope = {
  ...
  const prepared = (
    controller as unknown as { prepareFormModel: (value: Trait) => Trait }
  ).prepareFormModel(trait) as Trait;
...
describe('TraitEditorController validation workflow', () => {
  ...
  const emptyResult: TraitValidationResult = {
  ...
  it('applies auto fixes, updates the model and triggers validation', async () => {
    ...
    expect((controller as unknown as { canUndoAutoFix: () => boolean }).canUndoAutoFix()).toBe(
      true,
    );
  });
