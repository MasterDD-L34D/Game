...
export class TraitEditorController {
  ...
  validateCurrentTrait(): void {
    ...
    const baseline = this.trait
      ? mergeTrait(this.trait, this.formModel)
      : cloneTrait(this.formModel);
    ...
    this.dataService
      ...
      .catch((error: Error) => {
        ...
        const message =
          error?.message ??
          'Si è verificato un errore imprevisto durante la validazione del tratto.';
  ...
  confirm(editorForm: any): void {
    ...
    this.dataService
      ...
      .catch((error: Error) => {
        ...
        const message =
          error?.message ??
          'Si è verificato un errore imprevisto durante il salvataggio del tratto.';
  ...
  private loadTrait(id: string): void {
    ...
    this.dataService
      ...
      .then((trait) => {
        ...
        if (lastError) {
          this.stateService.setStatus(
            'Modifica locale: la sorgente remota non è stata raggiunta.',
            'info',
          );
        }
      })
      .catch((error: Error) => {
        ...
        this.stateService.setStatus(
          'Non è stato possibile preparare il tratto per la modifica.',
          'error',
        );
        this.trait = null;
        this.formModel = null;
  ...
  private resetValidationFeedback(options?: {
  ...
  private applyAutoFixOperations(
    model: TraitFormModel,
    fix: TraitValidationAutoFix,
  ): TraitFormModel {
