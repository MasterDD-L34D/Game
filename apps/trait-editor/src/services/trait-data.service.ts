...
export class TraitDataService {
  ...
  saveTrait(updatedTrait: Trait): Promise<Trait> {
    ...
    const persist = async (): Promise<{
      trait?: Trait;
      meta?: { etag: string | null; version: string | null };
    } | void> => {
      if (!this.useRemoteSource) {
        return;
      }
  ...
  private normaliseValidationAutoFix(
    source: unknown,
    fallbackId: string,
  ): TraitValidationAutoFix | null {
    if (!source || typeof source !== 'object') {
      return null;
    }
  ...
  private normaliseValidationAutoFixOperation(
    source: unknown,
  ): TraitValidationAutoFixOperation | null {
    if (!source || typeof source !== 'object') {
      return null;
    }
    ...
    const operation: TraitValidationAutoFixOperation = {
      op: opCandidate as TraitValidationAutoFixOperation['op'],
      path,
    };
  ...
  private normaliseValidationSummary(
    ...
    const errors =
      parseCount(
        record.errors ?? record.error ?? record.errorCount ?? record.errori ?? record.falli,
      ) ?? fallback.errors;
    const warnings =
      parseCount(record.warnings ?? record.warning ?? record.warningCount ?? record.avvisi) ??
      fallback.warnings;
    const suggestions =
      parseCount(
        record.suggestions ??
