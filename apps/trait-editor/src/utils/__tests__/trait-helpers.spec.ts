...
describe('trait helpers', () => {
  it('cloneTrait produces an independent copy of nested data', () => {
    ...
    expect(copy.entry.completion_flags.has_biome).not.toBe(
      original.entry.completion_flags.has_biome,
    );
  ...
  it('mergeTrait preserves untouched fields while applying nested changes', () => {
    ...
    expect(merged.entry.species_affinity[0].species_id).toBe(
      baseline.entry.species_affinity[0].species_id,
    );
