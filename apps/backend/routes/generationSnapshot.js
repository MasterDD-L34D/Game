...
function buildQualityRelease(baseQuality, diagnostics) {
  ...
  if (diagnostics && typeof diagnostics === 'object') {
    ...
    quality.traitDiagnosticsGeneratedAt =
      diagnostics.generated_at || diagnostics.generatedAt || null;
  } else {
...
function buildSnapshot({ dataset, diagnostics, runtime }) {
  ...
  snapshot.encounterSummary = computeEncounterSummary(
    snapshot.encounter,
...
function createGenerationSnapshotHandler(options = {}) {
  ...
  const snapshotStore = options.snapshotStore || createGenerationSnapshotStore({ datasetPath });
    ...
    try {
      ...
      if (traitDiagnosticsSync && typeof traitDiagnosticsSync.ensureLoaded === 'function') {
        try {
          ...
          diagnostics =
            typeof traitDiagnosticsSync.getDiagnostics === 'function'
