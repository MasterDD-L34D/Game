...
export class TraitService {
  ...
  async getTraitById(traitId: string): Promise<TraitDetailResponse> {
    ...
    const fallbackSource: TraitDataSource = preferMock ? 'mock' : 'fallback';
    const fallback = await this.loadDetail(
      this.config.mockDetailUrlTemplate,
      traitId,
      fallbackSource,
    );
    if (fallback.ok) {
  ...
  private async loadIndex(
    ...
    try {
      ...
      return {
        ok: false,
        error: new TraitServiceError('Errore sconosciuto durante il caricamento indice', {
          source,
        }),
      };
    }
  ...
  private async loadDetail(
    ...
    try {
      ...
      return {
        ok: false,
        error: new TraitServiceError('Errore sconosciuto durante il caricamento tratto', {
          source,
        }),
      };
    }
  ...
  private async postJson(
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Errore di rete durante la validazione del tratto';
  ...
  private normaliseValidationResponse(payload: unknown): TraitValidationResult {
    ...
    suggestions.forEach((entry, index) => {
      ...
      const message =
        typeof container.message === 'string' ? container.message : 'Suggerimento disponibile.';
