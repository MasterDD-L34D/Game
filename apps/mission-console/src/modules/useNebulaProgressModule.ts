import { computed, onBeforeUnmount, onMounted, ref, unref, type ComputedRef, type Ref } from 'vue';
...
} from '../services/fetchWithFallback.js';
...
function computeHistory(percent: number, qaPercent: number): number[] {
  ...
  return deduped.length >= 2
    ? deduped
    : [Math.max(Math.round(finalPoint * 0.6), 0), Math.max(Math.round(finalPoint), 0)];
}
...
export function useNebulaProgressModule(
  ...
  const dataSource = resolveDataSource('nebulaAtlas', {
    ...
    fallback: Object.prototype.hasOwnProperty.call(options, 'fallback')
      ? options.fallback
      : undefined,
    mock: Object.prototype.hasOwnProperty.call(options, 'telemetryMock')
      ? options.telemetryMock
      : undefined,
  });
  ...
  const telemetryEndpointCandidate = Object.prototype.hasOwnProperty.call(
    options,
    'telemetryEndpoint',
  )
    ? normaliseOptionalString(options.telemetryEndpoint)
    : deriveSegmentEndpoint(baseEndpoint, 'telemetry');
  const generatorEndpointCandidate = Object.prototype.hasOwnProperty.call(
    options,
    'generatorEndpoint',
  )
    ? normaliseOptionalString(options.generatorEndpoint)
    : deriveSegmentEndpoint(baseEndpoint, 'generator');
  const aggregateEndpointCandidate = Object.prototype.hasOwnProperty.call(
    options,
    'aggregateEndpoint',
  )
  ...
  const qualityRelease = computed(
    () => (unref(sources.qualityRelease) as Record<string, unknown>) || {},
  );
        ...
        const parsed = (() => {
          try {
            if (
              raw &&
              typeof raw === 'object' &&
              ('dataset' in raw || 'telemetry' in raw || 'generator' in raw)
            ) {
      ...
      try {
        ...
        const mapped =
          parseError instanceof ZodError
            ? fromZodError(parseError, 'Aggregato Nebula non valido', {
                code: 'nebula.aggregate.invalid',
              })
            : toServiceError(parseError, 'Aggregato Nebula non valido', {
                code: 'nebula.aggregate.invalid',
              });
  ...
  async function loadTelemetryMock(reason?: Error) {
    ...
    const parsed = (() => {
      try {
        ...
        if (parseError instanceof ZodError) {
          throw fromZodError(parseError, 'Telemetria mock non valida', {
            code: 'nebula.telemetry.invalid',
          });
        }
        throw toServiceError(parseError, 'Telemetria mock non valida', {
          code: 'nebula.telemetry.invalid',
        });
      }
    })();
    ...
    try {
      ...
      const seedFromAggregate = (aggregate: NebulaApiResponse) => {
        ...
        if (aggregate.telemetry) {
          ...
          lastUpdated.value =
            aggregate.telemetry.updatedAt || lastUpdated.value || new Date().toISOString();
        }
          ...
          const parsed = (() => {
            try {
              if (
                ...
                ('dataset' in payload || 'telemetry' in payload || 'generator' in payload)
              ) {
              ...
              if (parseError instanceof ZodError) {
                throw fromZodError(parseError, 'Dataset Nebula non valido', {
                  code: 'nebula.dataset.invalid',
                });
              }
              throw toServiceError(parseError, 'Dataset Nebula non valido', {
                code: 'nebula.dataset.invalid',
              });
            }
          })();
  ...
  const objectives = computed(() =>
    normaliseArray((overview.value as { objectives?: unknown })?.objectives),
  );
  const blockers = computed(() =>
    normaliseArray((overview.value as { blockers?: unknown })?.blockers),
  );

  const qaChecks = computed(() => {
    const checks =
      (qualityRelease.value as { checks?: Record<string, { passed?: unknown; total?: unknown }> })
        ?.checks || {};
  ...
  const cards = computed(() => {
    ...
    entries.push({
      ...
      tone:
        qaSummary.value.percent >= 80
          ? 'success'
          : qaSummary.value.percent >= 50
            ? 'warning'
            : 'neutral',
      progress: qaSummary.value.percent,
    });
    const completion =
      (overview.value as { completion?: { total?: unknown; completed?: unknown } })?.completion ||
      {};
  ...
  const timelineEntries = computed(() => {
    ...
    const referenceTime =
      qaSummary.value.lastRun || (timelineState.value as { lastSync?: string })?.lastSync || null;
    ...
    const species = Array.isArray(activeDataset.value.species)
      ? [...activeDataset.value.species]
      : [];
    species
      .sort((a, b) => {
