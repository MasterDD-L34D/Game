...
} from '../composables/useEventSource.ts';
...
function createEntry(
  ...
  defaults: Partial<ClientLogPayload> = {},
): ClientLogEntry {
  const scope = normaliseScope(
    (payload.scope as string | undefined) ?? (defaults.scope as string | undefined),
  );
  ...
  const requestId =
    ...
    null;
  const source =
    typeof payload.source === 'string' && payload.source.length
      ? payload.source
      : defaults.source || 'ui';
  const baseId =
...
function buildExport(
  entries: ClientLogEntry[],
  { filename, format }: { filename?: string; format?: LogExportFormat },
): LogExportResult {
  const resolvedFormat: LogExportFormat =
    String(format || 'json').toLowerCase() === 'csv' ? 'csv' : 'json';
...
function normaliseStreamEntry(
  payload: unknown,
  defaults: StreamEntryDefaults,
): ClientLogEntry | null {
  if (payload == null) {
    return null;
  }
  ...
  const event =
    typeof record.event === 'string' && record.event.length
      ? record.event
      : defaults.event || 'quality.stream.log';
  const entry = createEntry(event, {
    ...
    request_id:
      (record.request_id as string | null | undefined) ??
      (record.requestId as string | null | undefined),
    meta: record.meta,
...
function bindStreamHandler(options: ConnectStreamOptions) {
  ...
  streamDisposer = streamController.on(lastStreamOptions.event, (event) => {
    ...
    const entries = normaliseStreamPayload(payload, {
      ...
      event:
        event.type && event.type !== 'message' ? event.type : options.event || 'quality.stream.log',
    });
    entries.forEach((entry) =>
      appendEntry({ ...entry, source: entry.source || lastStreamOptions.source }),
    );
  });
}
...
export {
  ...
  DEFAULT_STREAM_URL as defaultStreamUrl,
};
