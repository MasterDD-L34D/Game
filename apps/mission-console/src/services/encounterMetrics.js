  ...
  return (variant.slots || []).map((slot) => ({
    ...
    species: (slot.species || []).map((specimen) => ({
      ...
      statistics: {
        ...(specimen.statistics || {}),
        threat_tier:
          specimen.statistics?.threat_tier ||
          specimen.threat_tier ||
          specimen.balance?.threat_tier ||
          null,
...
export function buildEncounterSuggestions(metrics) {
  ...
  if (!suggestions.length && threatTier !== 'T?') {
    suggestions.push(
      'La configurazione è bilanciata: valuta solo piccoli ritocchi narrativi o di pacing.',
    );
  }
