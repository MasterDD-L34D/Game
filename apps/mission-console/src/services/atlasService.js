...
function resolveAtlasEndpoints(options = {}) {
  ...
  const datasetEndpoint = resolveApiUrl(
    normaliseSegmentEndpoint(datasetCandidate, 'dataset', DEFAULT_DATASET),
  );
  const telemetryEndpoint = resolveApiUrl(
    normaliseSegmentEndpoint(telemetryCandidate, 'telemetry', DEFAULT_TELEMETRY),
  );
  const generatorEndpoint = resolveApiUrl(
    normaliseSegmentEndpoint(generatorCandidate, 'generator', DEFAULT_GENERATOR),
  );
...
async function fetchAtlasSegment(segment, options = {}) {
  ...
  const fallbackUrl =
    options.fallbackUrl ||
    options.fallback ||
    (options.allowLegacyFallback === false ? null : legacyFallback);
  ...
  const result = await fetchJsonWithFallback(endpoint, {
    ...
    fallbackInit: allowFallback
      ? { headers: { Accept: 'application/json' }, ...(options.fallbackInit || {}) }
      : undefined,
    allowFallback,
  ...
  const usedLegacyFallback =
    allowFallback && result.source === 'fallback' && fallbackUrl === legacyFallback;
