...
function normaliseValidation(payload = {}) {
  const corrected =
    payload && typeof payload.corrected === 'object' && payload.corrected !== null
      ? { ...payload.corrected }
      : null;
  const messages = Array.isArray(payload?.messages)
    ? payload.messages.filter(Boolean).map((message) => {
...
function normaliseRequest(payload = {}) {
  ...
  const fallbackTraitIds = normaliseStringList(
    payload.fallback_trait_ids || payload.fallbackTraits,
  );
  return {
    trait_ids: traitIds,
...
export async function generateSpeciesBatch(batchPayload, options = {}) {
  ...
  const errors = Array.isArray(payload?.errors)
    ? payload.errors.map((item) => toBatchError(item))
    : [];
