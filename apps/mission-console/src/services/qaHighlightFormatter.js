...
export function buildQaHighlightsSummary(payload, options = {}) {
  ...
  const matrixOnly = normaliseArray(
    highlights.matrix_only_traits || highlights.matrixOnlyTraits,
  ).length;
  const zeroCoverageArray = normaliseArray(
  ...
  const sections = sectionDefinitions.map((section) => {
    const source =
      highlights[section.key] ||
      highlights[section.key.replace(/_(.)/g, (_, char) => char.toUpperCase())];
  ...
  const topConflicts = normaliseArray(highlights.top_conflicts || highlights.topConflicts).map(
    (entry) => ({
