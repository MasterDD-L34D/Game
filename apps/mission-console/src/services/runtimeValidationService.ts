...
async function postRuntime(
  ...
  const parsed = (() => {
    try {
      ...
      if (parseError instanceof ZodError) {
        throw fromZodError(parseError, 'Risposta runtime non valida', {
          code: 'runtime.validation.invalid',
        });
      }
      throw toServiceError(parseError, 'Risposta runtime non valida', {
        code: 'runtime.validation.invalid',
      });
    }
  })();
  ...
  if (response.source === 'fallback') {
    meta.fallback_error = response.error
      ? response.error.message
      : 'Richiesta remota non disponibile';
  }
