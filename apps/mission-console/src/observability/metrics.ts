...
const metricsEnabled =
  metricsFlag === 'enabled' || (metricsFlag === 'auto' && defaultMetricsEnabled);

const metricsEndpoint = readEnvString('VITE_OBSERVABILITY_METRICS_ENDPOINT');
const metricsStorageKey =
  readEnvString('VITE_OBSERVABILITY_METRICS_STORAGE_KEY') || 'nebula:observability:metrics';
...
function recordMetric(
  kind: MetricEnvelope['kind'],
  payload: MetricDiagnosticPayload & { rating?: string; delta?: number; navigationType?: string },
) {
    ...
    const observer = new PerformanceObserver((list) => {
      list.getEntries().forEach((entry) => {
        if (entry.entryType === 'resource') {
          ...
          if (
            !['fetch', 'xmlhttprequest', 'script', 'link'].includes(
