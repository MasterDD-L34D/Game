...
const qualityReleaseSchema = z
  ...
  .transform((value) => {
    ...
    const markdown =
      typeof value.releaseNotesMarkdown === 'string' ? value.releaseNotesMarkdown : '';
    const html =
      typeof value.releaseNotesHtml === 'string'
...
export function parseFlowSnapshot<T extends Record<string, unknown>>(
