...
export function useSpeciesGenerator(options: SpeciesGeneratorOptions = {}) {
  ...
  async function runSpecies(
    ...
    if (!force && speciesCache.has(cacheKey)) {
      ...
      state.source =
        (cached.result?.meta?.endpoint_source as 'remote' | 'fallback' | undefined) || 'remote';
    ...
    try {
      ...
      state.source =
        (result.meta?.endpoint_source as 'remote' | 'fallback' | undefined) || 'remote';
  ...
  async function runSpeciesBatch(
    ...
    try {
      const result = (await generateSpeciesBatch(
        { batch: batchEntries },
        batchOptions,
      )) as SpeciesBatchResult;
  ...
  const meta: ComputedRef<Record<string, unknown>> = computed(() => state.result?.meta || {});
