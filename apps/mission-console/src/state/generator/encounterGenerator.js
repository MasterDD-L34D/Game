import {
  ENCOUNTER_BLUEPRINTS,
  getEncounterTemplate,
  getDefaultEncounterCopy,
} from './encounters.js';
...
function normalizeParameters(template, overrides = {}) {
  ...
  for (const parameter of template.parameters || []) {
    ...
    const selected =
      parameter.values.find((value) => value.value === defaultValue) || parameter.values[0];
...
function applyVariantQuantity(baseQuantity, variantQuantity) {
  ...
  return {
    min:
      variantQuantity.min ??
      (typeof baseQuantity === 'object' ? baseQuantity.min : (variantQuantity.max ?? 0)),
    max:
      variantQuantity.max ??
      (typeof baseQuantity === 'object' ? baseQuantity.max : (variantQuantity.min ?? 0)),
  };
}
...
function matchesSlotFilters(species, slotFilters, biomeId) {
  ...
  if (
    !slotFilters.roles.some(
      (role) => role === species.role_trofico || role === species.role || role === species.roleId,
    )
  ) {
    return false;
  }
  ...
  if (biomeId) {
    const biomes = new Set(
      [].concat(species.biomes || []).concat(species.spawn_rules?.biomes || []),
    );
...
function pickSpeciesForSlot(speciesPool, quantity, random) {
  ...
  for (let i = 0; i < quantity; i += 1) {
    ...
    const index = Math.floor(
      (typeof random === 'function' ? random() : Math.random()) * sorted.length,
    );
...
export function computeThreat(template, parameters, slotAssignments) {
  ...
  for (const assignment of slotAssignments) {
    ...
    const slotThreat = assignment.species.reduce((acc, specimen) => {
      const tier = parseThreatTier(
        specimen.statistics?.threat_tier || specimen.balance?.threat_tier,
      );
      return acc + tier;
    }, 0);
