...
export function useTraitDiagnostics(options: TraitDiagnosticsOptions = {}) {
  ...
  async function load({
    force = false,
    refresh = false,
  }: { force?: boolean; refresh?: boolean } = {}): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      state.fallbackLabel =
        state.source === 'fallback' ? determineFallbackLabel(endpointUrl || null) : null;
  ...
  const diagnostics: ComputedRef<Record<string, unknown>> = computed(() => state.diagnostics || {});
  ...
  const traitCatalog = computed(() => {
    const traits = Array.isArray(diagnostics.value?.traits) ? diagnostics.value.traits : [];
