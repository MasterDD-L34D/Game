...
export interface NavigationMetaPayload {
  ...
  breadcrumbs?: NavigationBreadcrumb[];
  tokens?: Array<
    | (Partial<NavigationToken> & { label?: string; id?: string; variant?: NavigationTokenVariant })
    | null
    | undefined
  >;
}
...
export function updateNavigationMeta(payload: NavigationMetaPayload = {}): void {
  ...
  const tokens = Array.isArray(payload.tokens)
    ? payload.tokens.map(normaliseToken).filter((entry): entry is NavigationToken => Boolean(entry))
    : [];
  state.tokens = tokens;
}
