...
export function createTimelinePreview(root) {
  ...
  return {
    render(events = [], options = {}) {
      ...
      if (usingFallback) {
        status.append(createElement('span', 'timeline-status__warning', 'Fallback manuale attivo'));
      }
      ...
      events.forEach((event) => {
        ...
        time.append(
          createElement('span', null, `Durata · ${formatDuration(event.durationMinutes)}`),
        );
        time.append(createElement('span', null, `Timezone · ${event.timezone}`));
        item.append(time);

        if (event.metadata?.description) {
          item.append(createElement('p', 'timeline-event__metadata', event.metadata.description));
        }

        if (event.tags?.length) {
          item.append(
            createElement('p', 'timeline-event__metadata', `Tag · ${event.tags.join(', ')}`),
          );
        }
        ...
        if (event.metadata?.rewards) {
          ...
          item.append(createElement('p', 'timeline-event__metadata', `Ricompense · ${rewards}`));
        }
        ...
        if (event.phases?.length) {
          ...
          event.phases.forEach((phase) => {
            ...
            phaseItem.append(
              createElement('span', null, `${phase.localStart} → ${phase.localEnd}`),
            );
            phaseItem.append(
              createElement(
                'span',
