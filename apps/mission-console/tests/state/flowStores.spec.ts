...
const dataSourceMock = vi.hoisted(() => {
  const defaults = new Map<
    string,
    { endpoint: string | null; fallback: string | null; mock: string | null }
  >([
    [
      'flowSnapshot',
      {
    ...
    [
      ...
      { endpoint: '/api/v1/generation/species/batch', fallback: null, mock: null },
    ],
    [
...
import {
  useSpeciesGenerator,
  __internals__ as speciesInternals,
} from '../../src/state/useSpeciesGenerator';
...
describe('flow stores', () => {
  ...
  describe('useSpeciesGenerator', () => {
    ...
    it('solleva errore se i trait mancano', async () => {
      const logger = { log: vi.fn() };
      const store = useSpeciesGenerator({ logger });
      await expect(store.runSpecies({ trait_ids: [] })).rejects.toThrow(
        'errors.species.trait_ids_required',
      );
    });
  });
