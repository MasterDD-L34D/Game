...
describe('parseFlowSnapshot', () => {
  it('sanitizza release notes e notifiche da markup potenzialmente pericoloso', () => {
          ...
          notifications: [
            {
              id: '1',
              channel: 'Ops',
              message: '<em onclick="doBad()">Check</em>',
              lastTriggeredAt: '2024-06-01T10:00:00Z',
            },
          ],
        },
      },
    ...
    expect(snapshot.qualityReleaseContext.releaseConsole.notifications[0].message).toContain(
      '<em>Check</em>',
    );
    expect(snapshot.qualityReleaseContext.releaseConsole.notifications[0].message).not.toContain(
      'onclick',
    );
  });
