...
## 🎮 Sprint context (aggiornato: 2026-04-28 — Sprint 12 Mating lifecycle wire shipped)

**Sessione 2026-04-28 (Sprint 12 — Mating lifecycle wire — Surface-DEAD #4 killer)**: 7° sprint consecutivo Engine LIVE Surface DEAD pattern proven. Engine LIVE in `metaProgression.js` (469 LOC, `rollMatingOffspring` + `canMate` + offspringRegistry da PR #1879) era invisibile in player loop normale. Surface DEAD chiuso wirando debrief panel post-victory.

**Backend** (`apps/backend/services/mating/computeMatingEligibles.js`, ~120 LOC): pure helper `computeMatingEligibles(survivors, encounterBiomeId, options)` filtra player team alive (controlled_by 'player'/'ally' + hp>0) + genera pair combinations n-choose-2 capped a 6 + opzionale `metaTracker.canMate()` gate (graceful fallback permissivo se tracker malformato/throws). Helper exports anche `filterPlayerSurvivors` + `pairCombinations` + `DEFAULT_MAX_PAIRS=6` + `DEFAULT_OFFSPRING_PER_PAIR=1`.

**Wire backend** (`apps/backend/services/rewardEconomy.js` `buildDebriefSummary`): aggiunto field `mating_eligibles[]` solo su victory + best-effort require pattern (mirror QBN narrativeEvent Sprint 10). Defeat/timeout → empty array. Biome reso dal session.encounter?.biome_id chain.

**Frontend** (`apps/play/src/lineagePanel.js`, ~90 LOC): pure `formatLineageCard` (HTML pair card con XSS escape) + `formatLineageList` (concat skip malformed) + side-effect idempotent `renderLineageEligibles(sectionEl, listEl, eligibles)`. Reuse `iconForBiome`+`labelForBiome` da `biomeChip.js`. Plural offspring label ("offsprings") quando count>1.

**Wire frontend**: `debriefPanel.js` HTML slot `<div id="db-lineage-section">` + `<div id="db-lineage-list">` + import + `setLineageEligibles(payload)` setter API + `renderLineage()` registered in `render()` (gated outcome non-defeat). `phaseCoordinator.js` pipe `bridge.lastDebrief.mating_eligibles` → `dbApi.setLineageEligibles(...)` quando phase 'debrief' (mirror QBN narrative event pattern).

**CSS scope** (`debriefPanel.css` `.db-lineage-card`): linear-gradient gold + serif italic + biome chip caps + offspring badge gold border. Visual style coerente con Sprint 10 QBN journal card ma palette differente (gold vs violet) per discriminare semantica diegetic vs lineage.

**Test budget**: 38/38 nuovi (19 backend `tests/services/computeMatingEligibles.test.js` + 19 frontend `tests/play/lineagePanel.test.js`). Coverage: filterPlayerSurvivors edge cases / pairCombinations n-choose-2 / metaTracker gate / null+empty+malformed graceful / XSS escape / biome icon fallback / offspring plural / idempotent render. AI baseline 382/382 verde zero regression. Format + governance verdi.

**Smoke E2E preview limitazione**: Vite dev server preview risolve root da repo principale (non worktree path), quindi non vede file nuovi in worktree. Validazione end-to-end via direct node integration test (`buildDebriefSummary` chiamata con session mock + assert `mating_eligibles[1]` con biome=savana, can_mate=true). Defeat path validated → empty array.

**Pillar impact**: P2 Evoluzione 🟢 def → 🟢++ (ciclo Nido→pair-bond→offspring visibile post-encounter, prima invisibile). §C.2 Surface-DEAD sweep: 6/8 → **7/8** (residuo solo #3 Spore mutation dots ~15h authoring esterno). Pattern proven 7× consecutive.
