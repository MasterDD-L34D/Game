...
1. **VERIFICA ground-truth (L-075 / anti-pattern #19)** — per ogni candidato BACKLOG/OD/COMPACT marcato ⏳/🟡/pending: `grep -rn <feature> <impl-dir>` + `git log origin/main --oneline | grep -i <ticket>` + check artifact (catalog stats / endpoint / test). Marker = ipotesi, git = verità. Già shipped → **chiudi entry (no rebuild)**, passa oltre.
