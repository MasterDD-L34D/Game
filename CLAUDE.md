...
- `apps/dashboard/` — Vite SPA scaffold (workspace `@game/dashboard`) wired against a **vendored AngularJS 1.8.3 stub** in `packages/angular*`. Data source registry lives at `apps/dashboard/src/config/dataSources.ts`, fallback JSON in `apps/dashboard/public/data/`. **See the architectural note below** before assuming `npm run dev` renders a working UI.
...
- **Dashboard reality check**. `npm run dev --workspace apps/dashboard` starts successfully and serves all TypeScript source files, but the `<app-root>` element **renders empty** — `packages/angular/index.js` is a 1.8.3-stub whose `bootstrap` only tags the root element with `__angularBootstrapped__` without mounting anything. The real production Mission Console bundle lives as pre-built assets under `docs/mission-console/` (`ConsoleLayout-*.js`, `FlowShellView-*.js`, `atlas-*.js`, `nebula-*.js`, `index.html`). If you need to verify UI changes end-to-end, you'll need to understand this dichotomy first: `apps/dashboard/` is the source scaffold, `docs/mission-console/` is the shipped artifact, and nothing today builds one from the other. Ask before investing in the dev-server smoke-test workflow — it's not plug-and-play.
