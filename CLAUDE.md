...
> **Current sprint** (2026-06-17 continuazione -- backlog gated eseguito, Lenovo): decision backlog presentato (AskUserQuestion) -> **4 verdetti eseguiti + 4 PR**. **A2 `pressure_tier_floor` LIVE in prod** (flip `export PRESSURE_TIER_FLOOR_ENABLED=true` keys.env + **deploy prod** worktree `b2b9ea51 -> 5927cb7d` perche' #2761 PRE-#2769 = floor data assente; verificato end-to-end Calm@0 -> Critical@75). #2782 **SPEC-A flip -> active (item-1 12/17)**. #2783 SPEC-F routes slices 0-2 (`/skiv/share` + `/crossbreed`-propose; slice 3 gated). #2784 OD-059 biome-memory (opt A reuse, read-only narrativo, +Codex P1 proto-pollution fixato). #2785 hc06 band-widen 0.15-0.25 -> **0.15-0.30** (opt A class-wide; combat-oracle green @N=100). **Flagged (verify-first)**: B2 gate-policy-random (dossier non committato), SPEC-F slice 3 (3 design-call), OD-059 narrative copy/soglie, **Postgres NON gira su Lenovo** (coop lobby hydrate degraded, pre-esistente). **Blocked-build residui**: B/F-slice3/G/H/J. main `2e0b9622`. Handoff: `docs/planning/2026-06-17-session-handoff-continuation.md`.
>
