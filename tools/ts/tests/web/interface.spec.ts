...
test("dashboard carica il dataset minimo senza errori", async ({ page, baseURL }) => {
  ...
  await expect(page.locator("[data-metric=\"species-synergies\"]")).toHaveText("1", { timeout: 15000 });
...
test("workspace manuale mette in coda il payload da packs.yaml", async ({ page, baseURL }) => {
  if (!baseURL) throw new Error("BaseURL mancante");
  const dataRoot = buildDataRoot(baseURL);
  await page.goto(`/docs/test-interface/manual-fetch.html?data-root=${encodeURIComponent(dataRoot)}`);
  ...
  const storedPayload = await page.evaluate(() => window.localStorage.getItem("et-manual-sync-payload"));
