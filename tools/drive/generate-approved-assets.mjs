...
async function main() {
  ...
  const configPath = path.resolve(
    repoRoot,
    args.config || 'config/drive/approved_asset_sources.json',
  );
  const outputPath = path.resolve(
    repoRoot,
    args.output || 'data/external/drive/approved_assets.json',
  );
  ...
  if (!Array.isArray(config.sources)) {
    console.error(
      'La configurazione deve contenere una proprietà "sources" con una lista di sorgenti.',
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (!args.quiet) {
    console.log(
      `Manifest generato: ${path.relative(repoRoot, outputPath)} (${assets.length} asset approvati)`,
    );
  }
}
...
function printUsage() {
  console.log(
    `Uso: generate-approved-assets [--config <path>] [--output <path>] [--quiet]\n\n` +
