...
def write_scenario_override(scenario_id, knob_values, yaml_path=DAMAGE_CURVES):
    ...
    REPLACES entire `scenario_overrides:` block with trial-only override.
    Original full yaml saved to caller for post-trial restore.

    BUG FIX 2026-05-21: previous regex-based partial-replace duplicated keys
    when existing scenario entry present — yaml parser took last (wrong)
    value, trial knobs silently ignored. New approach: full-block replace
    via line-by-line parser (no regex multiline traps).
    """
    with open(yaml_path, "r", encoding="utf-8") as f:
        original = f.read()

    # Build trial override block (clean, no inherited entries).
    block_lines = ["scenario_overrides:", f"  {scenario_id}:"]
    block_lines.append("    # OPTUNA TRIAL — auto-generated by calibrate_optuna.py")
    ...
    for i, line in enumerate(lines):
        if start_idx is None:
            # Start: line begins with "scenario_overrides:" at column 0
            if line.startswith("scenario_overrides:"):
                start_idx = i
                continue
        else:
            # End: next line at column 0 that's not empty AND not in our block.
            # Empty lines + indented lines + section markers stay inside block.
            stripped = line.rstrip()
            if not stripped:
                continue
            # First col char != whitespace AND != '#' (section comment usually
            # has special markers like "# ─── ..." which signals new section).
            if stripped[0] not in (" ", "\t"):
                # Check if section comment OR another top-level key.
    ...
    if start_idx is not None:
        # Replace block + 1 blank line separator.
        new_lines = lines[:start_idx] + trial_block.split("\n") + [""] + lines[end_idx:]
        modified = "\n".join(new_lines)
        if not modified.endswith("\n"):
            modified += "\n"
    else:
        # No existing block — insert before validation section.
        import re
        modified = re.sub(
            r"(\n# ─── Validation rules)",
            f"\n{trial_block}\n\\1",
