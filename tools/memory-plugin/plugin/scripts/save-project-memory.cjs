...
- Decisions: "chose Drizzle over Prisma for performance", "using RSC for data fetching"`;
  ...
  w.exports = { LocalMemoryClient: E, PERSONAL_ENTITY_CONTEXT: W, REPO_ENTITY_CONTEXT: Y };
});
var P = R((le, A) => {
  ...
  A.exports = { getGitRoot: H };
});
var M = R((ue, k) => {
  var d = require('node:fs'),
    j = require('node:path'),
    { getGitRoot: _ } = P(),
    O = j.join('.claude', '.supermemory-claude'),
    q = 'config.json';
  function x(e) {
    let n = _(e) || e;
    return j.join(n, O, q);
  ...
  k.exports = { getConfigPath: x, loadProjectConfig: I, saveProjectConfig: Q };
});
var L = R((ge, $) => {
  var { execSync: V } = require('node:child_process'),
    Z = require('node:crypto'),
    { loadProjectConfig: b } = M(),
    { getGitRoot: v } = P();
  function F(e) {
...
var { LocalMemoryClient: re, REPO_ENTITY_CONTEXT: me } = D(),
  { getRepoContainerTag: se, getProjectName: ie } = L();
async function ce() {
  ...
  try {
    ...
    (console.log(`Project knowledge saved: ${o}`), console.log(`ID: ${s.id}`));
  } catch (r) {
    console.log(`Error saving: ${r.message}`);
  }
}
ce().catch((e) => {
